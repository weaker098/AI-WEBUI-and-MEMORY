...
python -c "import keybert" >nul 2>nul
IF %ERRORLEVEL% NEQ 0 (
    echo  📦  Installing: keybert ^(session auto-naming^) ...
    SET /A MISSING_COUNT+=1
    python -m pip install keybert --quiet
    python -c "import keybert" >nul 2>nul
    IF %ERRORLEVEL% NEQ 0 ( echo  ❌  FAILED: keybert & SET /A FAILED_COUNT+=1 & SET FAILED_LIST=%FAILED_LIST% keybert ) ELSE ( echo  ✅  Installed: keybert & SET /A INSTALLED_COUNT+=1 )
) ELSE ( echo  ✅  keybert )
