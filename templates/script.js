        ...
        for (let i = 0; i < history.length; i++) {
            ...
            if (entry.role === "user" && !entry.silent) {
                    ...
                    if (!isVideoMessage) {
                        ...
                        if (imagePart) {
                            ...
                            if (imgUrl.startsWith('data:')) {
                                // FIX: Base64 data URLs cause the same lheading catastrophic
                                // backtrack as video frames — do NOT pass into marked.parse().
                                // Instead, inject the <img> directly into the DOM after render.
                                _pendingImageUrl = imgUrl;
                            } else {
                ...
                if (_pendingImageUrl) {
                    ...
                    if (msgContent) msgContent.appendChild(img);
                }
                userIndex++;
...
async function loadVideoSettings() {
    try {
        ...
        // Blob retention toggle
        _applyKeepBlobsToggle(!!s.keep_image_blobs);
...
async function saveVideoSettings() {
    const payload = {
        ...
        keep_image_blobs:  document.getElementById('keepImageBlobsToggle')?.checked ?? false,
    };
...
// Helper — syncs the visual toggle state for the blob retention switch.
// Called on load and when the user clicks the checkbox.
function _applyKeepBlobsToggle(on) {
    const cb    = document.getElementById('keepImageBlobsToggle');
    const thumb = document.getElementById('keepBlobsThumb');
    const label = document.getElementById('keepBlobsLabel');
    if (!cb) return;
    cb.checked = on;
    if (thumb) {
        thumb.style.left       = on ? '23px' : '3px';
        thumb.style.background = on ? '#4a90d9' : '#888';
        thumb.parentElement.querySelector('span:not(#keepBlobsThumb)').style.background = on ? '#1a3a5c' : '#333';
    }
    if (label) label.textContent = on ? 'ON — Vanilla (blobs kept in history)' : 'OFF — Ephemeral (default)';
}

// Wire live toggle feedback — runs once the DOM is ready (script is deferred to end of body).
document.addEventListener('DOMContentLoaded', () => {
    const cb = document.getElementById('keepImageBlobsToggle');
    if (cb) cb.addEventListener('change', () => _applyKeepBlobsToggle(cb.checked));
});
...
function resetVideoSettings() {
    ...
    _applyKeepBlobsToggle(false); // default is ephemeral (OFF)
    _updateVideoPreview();
