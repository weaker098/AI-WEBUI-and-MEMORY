...
function formatMarkdown(text, streamComplete = false) {
    ...
    // ^superscript^  →  <sup>
    // ~subscript~    →  <sub>
    // All run AFTER code-block restore so they never fire inside fences.
...
async function send() {
    ...
    for (const f of attachedFiles) {
        if (f.type.startsWith('image/')) {
            ...
            // skip — bubble will fire after extraction with correct frame count
        } else {
    ...
    if (videoFiles.length >= 1) {
        ...
        // Build payloadMsg: append [Attached File: name] for every non-image, non-video
        // file so the backend saves those tokens in history. On reloadChat they pass
        // through formatMarkdown step 0.05 and re-render as pills — no backend changes needed.
        let payloadMsg = msg;
        for (const f of attachedFiles) {
