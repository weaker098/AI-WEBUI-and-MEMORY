...
function formatMarkdown(text, streamComplete = false) {
    ...
    // 0.2. STREAMING INLINE MARKDOWN GUARD (FIX BUG 4 — tightened)
    ...
    //
    // Fix: use a simple ODD-COUNT check on the tail of the text. If a given marker token
    // appears an odd number of times in the last 400 chars, there is exactly one unclosed
    // opener — temporarily append a matching closer so marked renders the formatted version.
    // The closer is render-only — fullResponse is never mutated.
    //
    ...
    if (!streamComplete) {
        const _tail = processedText.slice(-400);

        // Count non-overlapping occurrences of a literal token in a string
        function _countToken(str, token) {
            let count = 0, pos = 0;
            while ((pos = str.indexOf(token, pos)) !== -1) { count++; pos += token.length; }
            return count;
        }

        // Longest tokens first — prevents * matching inside **
        const _guardTokens = ['**', '__', '~~', '*', '_'];
        for (const token of _guardTokens) {
            if (_countToken(_tail, token) % 2 !== 0) {
