...
function openSanityCheckPanel() {
    // Piggybacks on the Resonance modal (which already loads sanity check settings)
    // then scrolls straight to the Recall Filter section so the user lands in the right spot.
    openResonanceSettingsModal();
    // Wait for modal to be visible before scrolling
    requestAnimationFrame(() => {
        const target = document.getElementById('sanityCheckSettingsBody')
                    || document.getElementById('sanityCheckEnabledToggle');
        if (target) {
