...
function formatMarkdown(text, streamComplete = false) {
    ...
    // Fenced blocks extracted first (greedy match), then inline backtick spans.
    // Restored together after all replacements are done.
    const codeBlocks = [];
    let processedText = _safeText
        .replace(/```[\s\S]*?```/g, (match) => {
    ...
    if (!streamComplete) {
        ...
        if (_remainingFences && _remainingFences.length % 2 !== 0) {
            ...
            // Without this, the sealed content stays raw in processedText and patterns like
            // ==value== inside a streaming code block would get wrongly transformed.
...
async function send() {
    ...
    // 2. All image pills together on one line
    if (_sendImgFiles.length > 0) {
        displayMsg += '\n\n' + _sendImgFiles.map(f => `[Attached Image: ${f.name}]`).join(' ');
    }
    // 3. All image markdown below the pills (renders as grid when 2+)
    for (const f of _sendImgFiles) {
        if (f.serverFilename) {
            displayMsg += `\n\n![${f.name}](/get_image/${f.serverFilename})`;
        } else {
            displayMsg += `\n\n![${f.name}](${f.content})`;
        }
    }
                    ...
                    if (!isVideoMessage) {
                        ...
                        for (const imagePart of imageParts) {
                            ...
                            if (imgUrl.startsWith('data:')) {
                                ...
                                // Extract filename from URL for the alt text so the custom
                                // marked image renderer can show a proper filename pill.
                                const _imgName = imgUrl.split('/').pop().split('?')[0] || 'image';
                                contentToDisplay += `\n\n![${_imgName}](${imgUrl})`;
                            }
                        }
                    }
