...
marked.setOptions({
    ...
    headerIds: false,
    mangle: false,
    html: true    // FIX: Allow raw HTML (e.g. <em>, <strong>) from model output to pass through.
                  // Without this, marked escapes <em>word</em> → &lt;em&gt; (shows as literal text).
                  // DOMPurify runs after marked and is the real XSS gate — safe to enable.
});
...
// FIX BUG 5: Cache only COMPLETE (non-streaming) highlights.
// The old cache keyed on text content, which meant every partial streaming chunk
// created a unique cache entry, filling all 100 slots with useless partials and
// evicting useful entries for already-complete messages.
// New approach: the renderer is called with a _streamingActive flag set by
// formatMarkdown. During streaming we still highlight (for live color), but we
// skip the cache write. Only final renders (streamComplete=true) are cached.
const _highlightCache = new Map();
let _hlStreamingActive = false; // set true by formatMarkdown during streaming passes
marked.use({
    renderer: {
        code(token) {
            ...
            // ── Mermaid diagrams — hand off to mermaid.js instead of hljs ──
            if (lang === 'mermaid') {
                return `<div class="mermaid-wrapper"><pre class="mermaid">${text}</pre></div>`;
            }

            // Only consult/populate the cache for final (non-streaming) renders
            const cacheKey = `${lang}:${text}`;
            if (!_hlStreamingActive && _highlightCache.has(cacheKey)) {
            ...
            // Only write to cache on final renders — skip partial streaming chunks
            if (!_hlStreamingActive) {
                _highlightCache.set(cacheKey, result);
                // Keep last 200 entries (doubled — only complete blocks live here now)
        ...
        // Custom image renderer — wraps every chat-uploaded image in a figure
        // that shows a 📄 filename pill above the actual image. Alt text is the
        // filename (set in send() and reloadChat() for server URL images).
        image(token) {
            const href = (typeof token === 'object') ? (token.href || '') : (arguments[0] || '');
            const alt  = (typeof token === 'object') ? (token.text || token.title || '') : (arguments[2] || arguments[1] || '');
            const safeName = alt
                .replace(/&/g, '&amp;')
                .replace(/</g, '&lt;')
                .replace(/>/g, '&gt;')
            ...
            const safeHref = href
                ...
                .replace(/</g, '&lt;')
                .replace(/>/g, '&gt;');
            return `<div class="msg-img-figure"><img src="${safeHref}" alt="${safeName}" class="msg-img-chat"></div>`;
        }
    }
});
...
function formatMarkdown(text, streamComplete = false) {
    ...
    // FIX BUG 5: Tell the hljs renderer whether this is a streaming (partial) pass
    // so it skips caching partial code blocks. Restored to false in the finally block.
    _hlStreamingActive = !streamComplete;
    try {
    ...
    // 0. SAFEHOUSE: Extract code fences AND inline backtick code FIRST before any regex hunts.
    // Anything inside ``` ``` or `code` is a safehouse — no markup regex can enter.
    //
    // THINK_CODE LANE: Fenced blocks that appear INSIDE an unclosed think opener go into
    // :::THINK_CODE_N::: instead of :::CODE_BLOCK_N:::. This completely decouples the
    // think block from the main code pipeline — no hljs, no code-container, no Step 8,
    // no _extractCodeContainers involvement during streaming. After </think> closes and
    // _patchThoughtContainers does its final pass the content is re-rendered properly.
    //
    // Detection: scan for the last think opener before each fence. If an opener appears
    // after the most recent closer (or no closer exists), we are inside a think block.
    const codeBlocks     = [];   // :::CODE_BLOCK_N:::  — normal pipeline
    const thinkCodeBlocks = [];  // :::THINK_CODE_N:::  — dumb think-only renderer

    // Build opener/closer patterns from config (mirrors what Step 1 uses).
    const _tcOpenRx  = new RegExp(`<think>|\\[THINK\\]${thinkOpenToken  ? '|' + thinkOpenToken.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')  : ''}`, 'i');
    const _tcCloseRx = new RegExp(`</think>|</think}|\\[/THINK\\]${thinkCloseToken ? '|' + thinkCloseToken.replace(/[.*+?^${}()|[\]\\]/g, '\\$&') : ''}`, 'i');

    function _isInsideThink(text, fenceIndex) {
        // Returns true if fenceIndex is within an unclosed think block.
        const before = text.slice(0, fenceIndex);
        let depth = 0;
        // Walk through openers/closers in order to determine nesting at fenceIndex.
        const _evtRx = new RegExp(
            `(<think>|\\[THINK\\]${thinkOpenToken  ? '|' + thinkOpenToken.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')  : ''}|` +
            `</think>|</think}|\\[/THINK\\]${thinkCloseToken ? '|' + thinkCloseToken.replace(/[.*+?^${}()|[\]\\]/g, '\\$&') : ''})`,
            'gi'
        );
        let m;
        while ((m = _evtRx.exec(before)) !== null) {
            if (_tcOpenRx.test(m[0]))  depth++;
            if (_tcCloseRx.test(m[0])) depth = Math.max(0, depth - 1);
        }
        return depth > 0;
    }

    // Extract complete ``` pairs first — routed by think-context.
    let processedText = _safeText.replace(/```[\s\S]*?```/g, (match, offset, str) => {
        if (_isInsideThink(str, offset)) {
            const id = `:::THINK_CODE_${thinkCodeBlocks.length}:::`;
            thinkCodeBlocks.push(match);
            return id;
        }
    ...
    // Inline backticks — also routed by think-context.
    ...
    // 0.035. LATEX / MATH SAFEHOUSE — Extract math expressions before any other processing.
    // This runs BEFORE symbol shortcodes ($name$) so $$...$$ display blocks are grabbed
    // first and never mangled by the shortcode replacer or marked's table parser.
    //
    // Supported delimiters:
    //   \[...\]   → display math  (LaTeX standard block)
    //   $$...$$   → display math  (common alternative)
    //   \(...\)   → inline math   (LaTeX standard inline)
    //   $...$     → inline math   (runs AFTER $$, so double-$ is already gone)
    //              ↑ only fires when content has non-word chars (real math, not $word$)
    //
    // Restored and rendered via KaTeX AFTER DOMPurify — so KaTeX HTML is never stripped.
    const mathBlocks = [];
    const _saveMath = (src, display) => {
        const id = `:::MATH_${display ? 'D' : 'I'}_${mathBlocks.length}:::`;
        mathBlocks.push({ src: src.trim(), display });
        return display ? `\n\n${id}\n\n` : id;
    };
    // Display math: \[...\]  (multiline OK)
    processedText = processedText.replace(/\\\[([\s\S]*?)\\\]/g, (_, inner) => _saveMath(inner, true));
    // Display math: $$...$$
    processedText = processedText.replace(/\$\$([\s\S]*?)\$\$/g, (_, inner) => _saveMath(inner, true));
    // Inline math: \(...\)
    processedText = processedText.replace(/\\\(([\s\S]*?)\\\)/g, (_, inner) => _saveMath(inner, false));
    // Inline math: $...$ — only when the inner content looks like actual math
    // (contains an operator, backslash, digit, or subscript/superscript character)
    // so plain $word$ shortcodes are left for step 0.06 to handle.
    processedText = processedText.replace(/(?<!\$)\$(?!\$)([^$\n]+?)(?<!\$)\$(?!\$)/g, (match, inner) => {
        if (/[\\+\-*/^_=<>!{}\[\]|,0-9]/.test(inner)) return _saveMath(inner, false);
        return match; // leave for symbol shortcodes
    });

    // 0.04. ATTACHED IMAGE PILLS — Convert [Attached Image: name.ext] tokens into
    // styled inline 📄 pills. These tokens are written into both displayMsg (immediate)
    // and payloadMsg (saved to history text), so they persist through reloadChat.
    processedText = processedText.replace(/\[Attached Image: ([^\]]+)\]/g, (match, name) => {
        const safeName = name
            .replace(/&/g, '&amp;')
            .replace(/</g, '&lt;')
            .replace(/>/g, '&gt;')
            .replace(/"/g, '&quot;');
        return `<span class="file-attach-pill">📄 <span class="file-attach-pill-name" title="${safeName}">${safeName}</span></span>`;
    });

    // 0.05. ATTACHED FILE PILLS — Convert [Attached File: name.ext] tokens into
    // styled inline pills with a file-type icon emoji. Runs after code fence extraction
    // so any [Attached File: ...] that appears inside a code block stays verbatim.
    // These tokens are written into payload.message before the backend saves them, so
    // they survive reloadChat and page refreshes without needing any backend changes.
    processedText = processedText.replace(/\[Attached File: ([^\]]+)\]/g, (match, name) => {
        ...
        const icon = (
            ...
            ['zip', 'tar', 'gz', 'rar', '7z'].includes(ext)                                          ? '🗜️' :
            ['mp3', 'wav', 'ogg', 'flac', 'm4a'].includes(ext)                                       ? '🎵' :
            ['mp4', 'mov', 'avi', 'mkv', 'webm'].includes(ext)                                       ? '🎬' :
            ['js', 'ts', 'py', 'rb', 'go', 'java', 'cpp', 'c', 'h', 'cs', 'rs', 'swift', 'kt', 'php'].includes(ext) ? '💻' :
            ['json', 'xml', 'yaml', 'yml', 'toml', 'ini', 'cfg', 'env'].includes(ext)                ? '⚙️' :
        ...
        const safeName = name
            ...
            .replace(/>/g, '&gt;')
            .replace(/"/g, '&quot;');
        return `<span class="file-attach-pill">${icon} <span class="file-attach-pill-name" title="${safeName}">${safeName}</span></span>`;
    });

    // 0.06. SYMBOL SHORTCODES — $name$ → Unicode character.
    // Runs AFTER code fence extraction so shortcodes inside code blocks are untouched.
    // Unknown $foo$ is left verbatim — no false positives.
    const _SYMBOLS = {
        // arrows
        'arrow': '→', 'arrow-right': '→', 'rarr': '→',
        'arrow-left': '←', 'larr': '←',
        'arrow-up': '↑', 'uarr': '↑',
        'arrow-down': '↓', 'darr': '↓',
        'arrow-both': '↔', 'harr': '↔',
        ...
        'micro': 'µ', 'mu': 'µ',
        'sum': '∑', 'prod': '∏', 'int': '∫',
        'partial': '∂', 'nabla': '∇', 'delta': 'Δ',
        'alpha': 'α', 'beta': 'β', 'gamma': 'γ', 'lambda': 'λ',
        'sigma': 'σ', 'Sigma': 'Σ', 'theta': 'θ', 'phi': 'φ',
        'omega': 'ω', 'Omega': 'Ω',
        ...
        'music': '♪', 'Music': '♫',
        'sun': '☀', 'moon': '☽', 'cloud': '☁', 'snow': '❄', 'lightning': '⚡',
        'flag': '⚑', 'skull': '☠', 'peace': '☮', 'yin': '☯',
    };
    processedText = processedText.replace(/\$([a-zA-Z][a-zA-Z0-9_-]*)\$/g, (match, code) => {
        return _SYMBOLS[code] !== undefined ? _SYMBOLS[code] : match;
    });
    ...
    if (!streamComplete) {
        ...
        if (_remainingFences && _remainingFences.length % 2 !== 0) {
            ...
            // FIX BUG 2: Immediately re-extract the newly-sealed fence into the safehouse
            // so that step 1.9 inline markup (==, ^, ~) can NEVER fire inside it.
            // Route to THINK_CODE if the sealed fence is inside a think block.
            processedText = processedText.replace(/```[\s\S]*?```/g, (match, offset, str) => {
                if (_isInsideThink(str, offset)) {
                    const id = `:::THINK_CODE_${thinkCodeBlocks.length}:::`;
                    thinkCodeBlocks.push(match);
                    return id;
    ...
    // 0.2. STREAMING INLINE MARKDOWN GUARD (FIX: Prevents "seizures" and early closing)
    // During streaming, the AI may have opened an inline marker (**bold**, *italic*,
    // _italic_, __bold__, ~~strike~~) but not yet sent the closing token. marked.js
    // renders the unclosed marker as literal text, so the user sees raw asterisks or
    // underscores until the closer arrives, then it snaps to formatted text.
    //
    // OLD approach: odd-count check — caused "seizures" because a lone * was immediately
    // closed as ** (bold flash), and trailing spaces broke italics (*word * = raw text).
    //
    // NEW approach: find the LAST valid opener (preceded by space/start, followed by
    // non-space), check that it has actual content after it, then insert the closer
    // BEFORE any trailing whitespace so mid-sentence rendering stays clean.
    //
    // Priority order matters: check longer tokens (**/__/~~) before shorter ones (*/_ )
    // so a bold opener isn't mistakenly closed by an italic closer.
    if (!streamComplete) {
        // FIX: Skip the inline markdown guard entirely when we're inside an unclosed
        // thinking block. The AI uses bare asterisks (*) for bullet points inside think
        // blocks — treating them as italic openers causes the think closer to be orphaned
        // and triggers the flickering/seizure bug. Since think block content is raw prose,
        // not markdown, the guard has nothing useful to do there anyway.
        const _thinkOpenPartGuard  = thinkOpenToken  ? '|' + thinkOpenToken.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')  : '';
        const _thinkClosePartGuard = thinkCloseToken ? '|' + thinkCloseToken.replace(/[.*+?^${}()|[\]\\]/g, '\\$&') : '';
        const _openThinkCount  = (processedText.match(new RegExp(`<think>|\\[THINK\\]${_thinkOpenPartGuard}`,  'gi')) || []).length;
        const _closeThinkCount = (processedText.match(new RegExp(`<\\/think>|<\\/think\\}|\\[\\/THINK\\]${_thinkClosePartGuard}`, 'gi')) || []).length;
        const _insideThinkBlock = _openThinkCount > _closeThinkCount;

        // Use a slightly larger tail to catch long italicized sentences
        const _tail = processedText.slice(-1000);

        const _guardTokens = _insideThinkBlock ? [] : ['***', '___', '**', '__', '~~', '*', '_'];

        for (const token of _guardTokens) {
            const escaped = token.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');

            // Look for the LATEST occurrence of the token that acts as a valid OPENER.
            // A valid opener is: (Start or Whitespace) + Token + (Non-Whitespace)
            const openerRegex = new RegExp(`(^|\\s)${escaped}(?!\\s)`, 'g');
            let lastOpenerMatch = null;
            let m;
            while ((m = openerRegex.exec(_tail)) !== null) {
                lastOpenerMatch = m;
            }

            if (lastOpenerMatch) {
                const openerIdxInTail = lastOpenerMatch.index + lastOpenerMatch[1].length;
                const textAfterOpener = _tail.slice(openerIdxInTail + token.length);

                // If the token doesn't appear again after the opener, it's unclosed
                if (textAfterOpener.indexOf(token) === -1) {
                    const trailingMatch = textAfterOpener.match(/\s*$/);
                    const trailingWhitespace = trailingMatch ? trailingMatch[0] : '';
                    const mainContent = textAfterOpener.slice(0, textAfterOpener.length - trailingWhitespace.length);

                    // SEIZURE FIX: Only append the closer if there is actual content
                    // between the markers. This prevents "*" from becoming "**" instantly.
                    if (mainContent.length > 0) {
                        // Insert the closer BEFORE any trailing whitespace.
                        // This keeps italics/bold alive even while the AI is mid-sentence.
                        processedText = processedText.slice(0, processedText.length - trailingWhitespace.length) +
                                       token + trailingWhitespace;
                        break; // one closer per render frame is enough
                    }
                }
            }
        }
    ...
    // NEW STRATEGY (smart-model fix):
    //   - Search/recall headers and tool triggers are NO LONGER auto-closers.
    //     Smart models (e.g. Google) keep thinking INSIDE the think block when results
    //     arrive, only emitting the real closer at the very end. Force-closing at the
    //     result header would orphan the real closer and corrupt the render.
    //   - Instead we use ONE rule only: a NEW opening think tag appearing after content
    //     acts as the closer for the previous block. This handles dumb models that forget
    //     to close (they re-open after results) while leaving smart models untouched.
    //   - Search/recall widgets and results are simply absorbed inside the thought box.
    //     They render fine there — no problem.
    {
        ...
        // Find every think opener and check if the block is still unclosed when a NEW
        // opener appears. If so, inject a </think> just before the new opener.
        const openerRx = new RegExp(`(<think>|\\[THINK\\]${_customOpenEsc})`, 'gi');
        let _acMatch;
        const _openerPositions = [];
        while ((_acMatch = openerRx.exec(processedText)) !== null) {
            _openerPositions.push({ idx: _acMatch.index, tag: _acMatch[0] });
        }

        // Walk opener pairs: if block[N] has no closer before block[N+1], inject one
        if (_openerPositions.length >= 2) {
            // Iterate in reverse so injections don't shift earlier positions
            for (let _i = _openerPositions.length - 1; _i >= 1; _i--) {
                const _prev = _openerPositions[_i - 1];
                const _curr = _openerPositions[_i];
                const _between = processedText.slice(_prev.idx + _prev.tag.length, _curr.idx);
                if (!closeCheck.test(_between)) {
                    // Unclosed block — inject </think> just before the new opener
                    processedText =
                        processedText.slice(0, _curr.idx) +
                        '</think>\n\n' +
                        processedText.slice(_curr.idx);
                }
            }
        }
    }
    ...
    // 0.9. CODE FENCE GUARD INSIDE THINK BLOCKS
    // The AI sometimes writes code blocks while thinking (e.g. drafting code snippets
    // mid-reasoning). During streaming, the unclosed fence inside the think block causes
    // the think regex's lazy [\s\S]*? to re-evaluate its endpoint every chunk — the think
    // box flickers open/closed as the fence content grows. 9-month bug. The fix: just
    // flatten all code fences inside think blocks to plain text before the think regex runs.
    // The AI is THINKING — it doesn't need syntax highlighting in its scratchpad.
    // This is render-only — fullResponse is never mutated.
    if (!streamComplete) {
        const _thinkOpenPartGuard  = thinkOpenToken  ? '|' + thinkOpenToken.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')  : '';
        const _thinkClosePartGuard = thinkCloseToken ? '|' + thinkCloseToken.replace(/[.*+?^${}()|[\]\\]/g, '\\$&') : '';
        const _openRxGuard  = new RegExp(`<think>|\\[THINK\\]${_thinkOpenPartGuard}`,  'i');
        const _closeRxGuard = new RegExp(`<\\/think>|<\\/think\\}|\\[\\/THINK\\]${_thinkClosePartGuard}`, 'i');
        // Only act if we're inside an unclosed think block
        if (_openRxGuard.test(processedText) && !_closeRxGuard.test(processedText)) {
            // Strip all code fences (``` ... ``` complete pairs AND orphaned openers) to plain text
            processedText = processedText
                .replace(/```[^\n]*\n([\s\S]*?)```/g, '$1')   // complete fenced blocks → content only
                .replace(/```[^\n]*\n([\s\S]*?)$/g,  '$1')    // orphaned opener → content only
                .replace(/```[^\n]*/g, '');                     // any remaining fence line → gone
        }
    }
    ...
    processedText = processedText.replace(thinkRegex, (match, content) => {
        ...
        // CHECKPOINT LOGIC: Even if the think block has no closing tag, it's NOT a failure
        // if a tool was fired or a search/recall was triggered mid-think. The model didn't
        // fail — it left to go find answers. The sentinel/headers are proof of life.
        // IMPORTANT: Check `text` (the raw original), NOT `processedText` — by this point
        // in the pipeline the tool trigger tokens have already been consumed and converted
        // into blockquote widgets, so they no longer exist in processedText.
        const _hasCheckpoint = (
            text.includes(':::TOOL_FIRED:::') ||
            (searchToolTrigger  && text.includes(searchToolTrigger))  ||
            (recallToolTrigger  && text.includes(recallToolTrigger))  ||
            (searchResultHeader && text.includes(searchResultHeader)) ||
            (recallResultHeader && text.includes(recallResultHeader))
        );
        const failPrefix = (!isClosed && !_hasCheckpoint && catchFailedThinking && streamComplete) ? ':::THINK_FAILED:::' : '';

        return `\n\n:::THOUGHT_START:::${failPrefix}${trimmedContent}:::THOUGHT_END:::\n\n`;
    });

    // 1.9. INLINE MARKUP EXTENSIONS
    // ==highlight==  →  <mark>
    // ^superscript^  →  <sup>
    // ~subscript~    →  <sub>
    // NOTE: These run BEFORE code-block restoration (step 1b) so they NEVER fire
    // inside fenced or inline code content. Code blocks are still placeholders here —
    ...
    // Must happen AFTER all regex replacements above — code block content is untouched.
    codeBlocks.forEach((block, i) => {
        processedText = processedText.replace(`:::CODE_BLOCK_${i}:::`, block);
    });

    // 1c. THOUGHT SAFEHOUSE — Pre-render thought block content with marked separately.
    // Problem: marked.js sees "    *   item" (4-space-indented asterisk list) inside the
    // :::THOUGHT_START::: placeholder and renders it as a GFM list, then hljs wraps it in
    // <pre><code>. Fix: extract content, run marked.parse() on it NOW (formatting intact),
    // store the resulting HTML, replace with an index token, then re-inject after the main
    // marked.parse() pass. Formatting preserved, code block bug gone.
    const thoughtSafehouses = [];
    processedText = processedText.replace(
        /:::THOUGHT_START:::([\s\S]*?):::THOUGHT_END:::/g,
        (match, content) => {
            const idx = thoughtSafehouses.length;
            // Pre-render with marked so bullet/bold/italic formatting is preserved,
            // but the content never gets double-parsed by the main marked.parse() below.
            // Strip leading spaces/tabs before asterisk bullets so marked sees
            // "* item" not "    *   item" — the indented form triggers GFM list-as-code.
            //
            // CRITICAL: Strip :::THINK_FAILED::: prefix BEFORE marked.parse() — if we
            // pass it through marked it gets wrapped in <p> tags and the isFailed check
            // at the render stage (content.startsWith(':::THINK_FAILED:::')) will never
            // match because the content starts with '<p>' instead. Re-attach after parse.
            const _failedMarker = ':::THINK_FAILED:::';
            const _hasFailed = content.startsWith(_failedMarker);
            const _rawContent = _hasFailed ? content.slice(_failedMarker.length) : content;

            // THINK_CODE RESTORATION: Replace :::THINK_CODE_N::: placeholders with plain
            // <pre class="think-code"> blocks BEFORE marked.parse() runs on this content.
            // This means marked never sees a raw ``` fence, so it can NEVER eat
            // :::THOUGHT_END::: as code content. The boundary is always safe.
            // Dumb, minimal blocks — no hljs, no copy button, no code-container —
            // completely invisible to _extractCodeContainers during streaming.
            let safeContent = _rawContent.replace(/:::THINK_CODE_(\d+):::/g, (_, n) => {
                const raw = thinkCodeBlocks[parseInt(n)];
                if (raw === undefined) return '';
                // Fenced block — extract language hint and body
                const _fenceMatch = raw.match(/^```([^\n]*)\n?([\s\S]*?)```$/s);
                if (_fenceMatch) {
                    const lang = (_fenceMatch[1] || '').trim();
                    const body = _fenceMatch[2]
                        .replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
                    const langLabel = lang
                        ? `<span class="think-code-lang">${lang.replace(/</g, '&lt;')}</span>`
                        : '';
                    return `\n\n<pre class="think-code">${langLabel}<code>${body}</code></pre>\n\n`;
                }
                // Inline backtick — wrap in <code>
                const _inlineMatch = raw.match(/^`([^`]+)`$/);
                if (_inlineMatch) {
                    const body = _inlineMatch[1]
                        .replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
                    return `<code class="think-inline-code">${body}</code>`;
                }
                return raw;
            });

            const dedented = safeContent.trim()
                .replace(/^(    |\t)/gm, '')  // strip 4-space/tab indents — prevents marked from routing them to the old hljs code renderer
                .replace(/^[ \t]+\*/gm, '*'); // existing bullet fix
            const preRendered = (_hasFailed ? _failedMarker : '') + marked.parse(dedented);
            thoughtSafehouses.push(preRendered);
            return `:::THOUGHT_START:::${idx}:::THOUGHT_END:::`;
        }
    );

    // 3. Main Markdown Parse
    // VIDEO FRAMES TAG: Replace [Video frames: X frames from 'Y'] with a styled badge
    processedText = processedText.replace(
        /\[Video frames:\s*([^\]]+)\]/g,
        (_, label) => `<span class="video-frames-tag">🎬 ${label.trim()}</span>`
    );

    let html = marked.parse(processedText);

    // 3.5. MULTI-IMAGE GRID — When two or more consecutive msg-img-figure blocks
    // appear, wrap them in a flex-row grid so they display side by side.
    html = html.replace(/(<div class="msg-img-figure">[\s\S]*?<\/div>\s*){2,}/g, match =>
        `<div class="msg-img-grid">${match}</div>`
    );

    // 4. TRANSFORM PLACEHOLDERS TO UI CONTAINERS
    // Now we turn those protected thoughts into the actual HTML boxes.
    // Restore safehoused thought content (index tokens survived marked.parse intact).
    html = html.replace(/:::THOUGHT_START:::(\d+):::THOUGHT_END:::/g, (match, idx) => {
        const content = thoughtSafehouses[parseInt(idx)] || '';
        return `:::THOUGHT_START:::${content}:::THOUGHT_END:::`;
    });
    ...
    // 6.5. MATH RENDERING — Replace math placeholders with KaTeX-rendered HTML.
    // Runs AFTER DOMPurify so KaTeX output is never sanitized away.
    if (mathBlocks.length > 0 && typeof katex !== 'undefined') {
        cleanHtml = cleanHtml.replace(/:::MATH_([DI])_(\d+):::/g, (match, type, idx) => {
            const { src, display } = mathBlocks[parseInt(idx)];
            try {
                return katex.renderToString(src, {
                    displayMode: display,
                    throwOnError: false,   // renders error message inline, never throws
                    trust: false,
                    output: 'html'         // pure HTML — no MathML, nothing for DOMPurify to strip
                });
            } catch (e) {
                // Graceful fallback: show the raw LaTeX in a code span
    ...
    // THINKING BLOCK GUARD: Shelter all .thought-container blocks before Step 8 runs so
    // the gray .code-container wrapper is NEVER injected inside a thinking block.
    // Any <pre><code> that marked produced inside a thought (e.g. from 4-space indented text
    // or a fenced block the AI wrote while reasoning) must stay untouched — the thinking
    // block has its own black .think-code renderer. Step 8 is for the outside world only.
    const _thoughtSlots = [];
    cleanHtml = cleanHtml.replace(/<div class="thought-container[\s\S]*?<\/div>\s*<\/div>\s*<\/div>/g, (m) => {
        const slot = `:::THOUGHT_SLOT_${_thoughtSlots.length}:::`;
        _thoughtSlots.push(m);
        return slot;
    });
    ...
    let finalHtml = cleanHtml.replace(codeBlockRegex, (match, attrs) => {
    ...
    // Restore sheltered thought blocks — completely untouched by Step 8.
    finalHtml = finalHtml.replace(/:::THOUGHT_SLOT_(\d+):::/g, (_, i) => _thoughtSlots[parseInt(i)] || '');

    return finalHtml;

    } finally {
        // Always restore the streaming flag so a thrown error doesn't leave it stuck true
        _hlStreamingActive = false;
    }
}
...
// Patch edit/delete buttons onto the two most recent live chat bubbles
// (user + assistant) without requiring a full reloadChat().
// Called after each successful stream so new messages get their buttons
// the moment generation ends — identical logic to what reloadChat/appendMessage does.
async function _patchLatestBubbleButtons(assistantBubble) {
    try {
        const res = await fetch('/history');
        if (!res.ok) return;
        const history = await res.json();

        // --- Find last non-silent user message ---
        let userPosIndex = -1;   // positional (0-based) among user msgs → matches reloadChat's userIndex arg
        let userHistIdx  = -1;   // history array index → memIndex for openEditMessageModal
        let userMsgId    = null;
        let userCount    = 0;
        for (let i = 0; i < history.length; i++) {
            const e = history[i];
            if (e.role === 'user' && !e.silent) {
                userPosIndex = userCount;
                userHistIdx  = i;
                userMsgId    = e.msg_id || null;
                userCount++;
            }
        }

        // --- Find last non-silent assistant message ---
        let asstHistIdx = -1;
        let asstMsgId   = null;
        for (let i = history.length - 1; i >= 0; i--) {
            const e = history[i];
            if (e.role === 'assistant' && !e.silent) {
                asstHistIdx = i;
                asstMsgId   = e.msg_id || null;
                break;
            }
        }
        ...
        // ── Patch user bubble (last .chat-bubble.user in the log) ────────
        if (userHistIdx >= 0) {
            const userBubbles = chatlog.querySelectorAll('.chat-bubble.user');
            const lastUserBubble = userBubbles[userBubbles.length - 1];
            if (lastUserBubble && !lastUserBubble.querySelector('.msg-actions')) {
                const idAttr  = userMsgId ? `, '${userMsgId}'` : '';
                const editBtn = `<button class="msg-action-btn" onclick="openEditMessageModal(${userHistIdx})" title="Edit">✏️</button>`;
                const delBtn  = `<button class="msg-action-btn delete" onclick="deletePair(${userPosIndex}${idAttr})" title="Delete">🗑</button>`;
                const actionsDiv = document.createElement('div');
                actionsDiv.className = 'msg-actions';
                actionsDiv.innerHTML = editBtn + delBtn;
                lastUserBubble.appendChild(actionsDiv);
            }
        }

        // ── Patch assistant bubble ───────────────────────────────────────
        if (assistantBubble && !assistantBubble.querySelector('.msg-actions')) {
            const editBtn = asstHistIdx >= 0
                ? `<button class="msg-action-btn" onclick="openEditMessageModal(${asstHistIdx})" title="Edit">✏️</button>`
                : '';
            const delBtn = asstMsgId
                ? `<button class="msg-action-btn delete" onclick="deletePair(null, '${asstMsgId}')" title="Delete">🗑</button>`
                : '';
            if (editBtn || delBtn) {
                const actionsDiv = document.createElement('div');
                actionsDiv.className = 'msg-actions';
        ...
        console.warn('_patchLatestBubbleButtons failed silently:', e);
    }
}
...
async function streamAssistantResponse(fetchPromise, assistantBubble, signal = null) {
    ...
    //
    // FIX BUG 3 — CODE BLOCK FLICKER: The old approach wiped contentSpan.innerHTML on
    // every single token, destroying and recreating the entire .code-container (border,
    // rounded corners, lang label, copy button, pre block) ~every 10ms. This caused the
    // visible "malforming" flash. The new approach:
    //   1. Splits the new HTML into a "pre-code" text node + zero or more code containers.
    //   2. For each code container slot, checks if the live DOM already has one there.
    //   3. Only replaces the code container if its inner <code> content has actually changed.
    //   4. Everything outside code containers (the prose text) is still set via innerHTML
    //      but on a lightweight wrapper that excludes the stable containers.
    // Result: the .code-container shell (box, border, buttons) is never touched once created
    // — only the text cursor position inside <code> advances.
    let _lastRenderedHtml = '';
    function _renderAndRestoreStates(html) {
        // Short-circuit if nothing changed (can happen with caching)
        if (html === _lastRenderedHtml) return;
        _lastRenderedHtml = html;

        // FIX LAYOUT SHIFT: Snapshot distance-from-bottom before any DOM mutation.
        const distanceFromBottom = chatlog.scrollHeight - chatlog.scrollTop - chatlog.clientHeight;

        // --- SMART CODE-CONTAINER DIFF ---
        // .code-container must NEVER leave contentSpan — detaching it resets pre.scrollLeft
        // to 0, killing the user's horizontal scroll position mid-stream.
        // Fix: update <code> innerHTML in-place, rebuild prose AROUND the anchored
        // containers via insertBefore. The live node never moves.
        function _extractCodeContainers(src) {
            const CODE_OPEN = '<div class="code-container">';
            const blocks = [];
            let sf = 0;
            while (true) {
                const oi = src.indexOf(CODE_OPEN, sf);
                if (oi === -1) break;
                let depth = 0, i = oi;
                while (i < src.length) {
                    if (src.startsWith('<div', i)) depth++;
                        ...
                        depth--;
                        if (depth === 0) { blocks.push({ html: src.slice(oi, i + 6) }); sf = i + 6; break; }
                    }
                    i++;
                }
                if (depth !== 0) break;
        ...
        const newContainers = _extractCodeContainers(html);
        const liveContainers = contentSpan.querySelectorAll('.code-container');

        if (newContainers.length === 0 || newContainers.length !== liveContainers.length) {
            // Container count changed — full rebuild.
            contentSpan.innerHTML = html;
        } else {
            // Step 1 — update <code> innerHTML in place. liveContainer stays in DOM.
            liveContainers.forEach((live, idx) => {
                const t = document.createElement('div');
                t.innerHTML = newContainers[idx].html;
                const liveCode = live.querySelector('code');
                const newCode  = t.querySelector('code');
                if (liveCode && newCode && liveCode.innerHTML !== newCode.innerHTML)
                    liveCode.innerHTML = newCode.innerHTML;
            });

            // Step 2 — replace containers with sentinels, parse prose.
            let proseSrc = html;
            newContainers.forEach((blk, idx) => {
                proseSrc = proseSrc.replace(blk.html, `<div class="code-container-sentinel" data-idx="${idx}"></div>`);
            });
            const tmp = document.createElement('div');
            tmp.innerHTML = proseSrc;

            // Step 3 — ordered plan: live slot or prose node.
            const liveSet = new Set([...liveContainers]);
            const items = [];
            tmp.childNodes.forEach(node => {
                if (node.nodeType === 1 && node.classList.contains('code-container-sentinel'))
                    items.push({ isLive: true, idx: +node.dataset.idx });
                else
                    items.push({ isLive: false, node });
            });

            // Step 4 — strip only prose; anchors stay.
            [...contentSpan.childNodes].forEach(c => { if (!liveSet.has(c)) contentSpan.removeChild(c); });

            // Step 5 — re-insert prose around anchors.
            // FIX: Guard ref against stale detached nodes. If a prior render cycle did a
            // full innerHTML rebuild, liveContainers nodes may no longer be children of
            // contentSpan — insertBefore on a detached ref throws NotFoundError.
            // Falling back to null = append-at-end, which is always correct here.
            let li = 0, ref = liveContainers[0] || null;
            items.forEach(item => {
                if (item.isLive) { li++; ref = liveContainers[li] || null; }
                else {
                    if (ref && ref.parentNode !== contentSpan) ref = null;
                    contentSpan.insertBefore(item.node, ref);
                }
            });
        }

        // Re-render any mermaid diagrams that just landed in the DOM
        if (typeof mermaid !== 'undefined') {
            mermaid.run({ nodes: contentSpan.querySelectorAll('.mermaid') });
        }
        ...
        contentSpan.querySelectorAll('.thought-container').forEach((el, i) => {
            ...
            if (savedStates[i]) {
                ...
                if (wrapper) {
                    ...
                    // STREAMING ANIMATION: scroll collapsed viewport to bottom each frame
                    // so latest thought text flows through the vignette window like a live feed.
    ...
    function _foldOrphanedReasoningIntoThink(text) {
        ...
        // SMART MODEL GUARD: If a custom thinkCloseToken is configured, the model manages
        // its own think lifecycle (e.g. Google's <|channel> / <channel|> tags). It keeps
        // thinking through tool results and only emits its closer at the very end.
        // Force-closing here would orphan the real closer — skip it and let the stream
        // continue naturally. Step 0.5 in formatMarkdown handles the new-opener rule.
        if (thinkCloseToken) {
            console.log('🔧 TOOL_FIRED: Smart model detected (custom thinkCloseToken) — skipping force-close, absorbing into think block');
            return text;
        }

        // *** THE FIX: Force-close the unclosed think block ***
        // For standard models (<think>/<\/think>) that forget to close when a tool fires.
    ...
    try {
        ...
        // POST-STREAM THOUGHT PATCH ───────────────────────────────────────────────
        // formatMarkdown during streaming uses streamComplete=false, so failed-think
        // detection never fires mid-stream (intentional). Now that the stream is done,
        // run one final pass with streamComplete=true and surgically swap ONLY the
        // thought containers. No reloadChat, no scroll jump, nothing else changes.
        // All existing logic (TOOL_FIRED closing, bootleg closing tags, etc.) is intact
        // because this just calls the same formatMarkdown you already wrote.
        (function _patchThoughtContainers() {
            const finalHtml = formatMarkdown(fullResponse, true);
            const tmp = document.createElement('div');
            tmp.innerHTML = finalHtml;
            const newThoughts  = [...tmp.querySelectorAll('.thought-container')];
            const liveThoughts = [...contentSpan.querySelectorAll('.thought-container')];
            if (newThoughts.length !== liveThoughts.length) return; // count mismatch — bail
            liveThoughts.forEach((live, i) => {
                if (live.outerHTML !== newThoughts[i].outerHTML)
                    live.replaceWith(newThoughts[i]);
            });
        })();
...
async function send() {
    ...
    let displayMsg = msg;
    const _hasVideo = attachedFiles.some(f => f.type.startsWith('video/'));

    // Separate images from other files so the layout matches reloadChat:
    // reloadChat renders pills from the saved text content (all grouped) and
    // injects images via DOM after — so on immediate send we also group all
    // pills first then all images, rather than interleaving them per-file.
    const _sendImgFiles   = attachedFiles.filter(f => f.type.startsWith('image/'));
    const _sendOtherFiles = attachedFiles.filter(f => !f.type.startsWith('image/') && !f.type.startsWith('video/'));

    // 1. Non-image file pills (unchanged behaviour)
    for (const f of _sendOtherFiles) {
        displayMsg += `\n\n[Attached File: ${f.name}]`;
    }
    // 2. Image pills — name hidden, image still renders via markdown below
    // (pill text intentionally blank so no filename shows in the user bubble)
    // 3. All image markdown below (renders as grid when 2+), alt text blank to hide filename
    for (const f of _sendImgFiles) {
        if (f.serverFilename) {
            displayMsg += `\n\n![ ](/get_image/${f.serverFilename})`;
        } else {
            displayMsg += `\n\n![ ](${f.content})`;
        }
    }

    if (!_hasVideo && displayMsg.trim()) {
    ...
    // For video path the user bubble fires AFTER extraction, so defer appending
    // the assistant bubble until then — otherwise they appear in wrong order.
    if (!_hasVideo) chatlog.appendChild(assistantBubble);
    ...
    if (videoFiles.length >= 1) {
        ...
        //   + image files         → sent as extra_image_N form fields
        ...
        // SERVER-SIDE EXTRACTION: upload raw video to /upload_video.
        // The server uses OpenCV to extract frames, bypassing mobile browser
        // canvas restrictions that produce black frames in mobile mode.
        let _vs = {};
        try { _vs = await fetch('/get_video_settings').then(r => r.json()); } catch(_) {}

        showToast(`🎬 Uploading '${videoFile.name}' for server-side extraction...`, 'info', 4000);

        const _formData = new FormData();
        _formData.append('video',         videoFile._videoFile);   // raw File object
        _formData.append('prompt',        (msg || "Describe what happens in this video clip in detail.") + contextBlock);
        _formData.append('frame_divisor', _vs.frame_divisor ?? 2);
        _formData.append('frame_floor',   _vs.frame_floor   ?? 8);
        _formData.append('frame_cap',     _vs.frame_cap     ?? 200);
        _formData.append('jpeg_quality',  _vs.jpeg_quality  ?? 0.82);
        _formData.append('resolution',    _vs.resolution    ?? 720);

        // Standalone images bundled with the video
        if (imageFiles.length > 0) {
            imageFiles.forEach((imgFile, idx) => {
                _formData.append(`extra_image_${idx}`, imgFile.content);
            });
            _formData.append('extra_image_count', imageFiles.length);
            showToast(`🖼 Bundling ${imageFiles.length} image(s) with video.`, 'info', 2500);
        }

        // Show user bubble immediately — exact frame count comes back in memory on reload.
        // The [Video frames:] tag is enough for the reload guard (isVideoMessage check).
        const fpsLabel = `from '${videoFile.name}'` +
            (imageFiles.length > 0 ? ` + ${imageFiles.length} image(s)` : '');
        const _videoDisplayMsg = displayMsg + `\n\n[Video frames: ${fpsLabel}]`;
        appendMessage(currentLoadoutDisplayName, _videoDisplayMsg.trim(), "user");
        // Assistant bubble goes in AFTER user bubble so order is correct
        chatlog.appendChild(assistantBubble);

        fetchPromise = fetch("/upload_video", {
            method: "POST",
            body:   _formData,
            signal
        });
        ...
        const imagePayload = imgFile.serverFilename
            ? { prompt: msg, server_filename: imgFile.serverFilename, original_filename: imgFile.name }
            : { prompt: msg, image_data: imgFile.content, original_filename: imgFile.name };
        ...
        // Build payloadMsg: non-image files get [Attached File: name] tokens,
        // images get [Attached Image: name] tokens grouped on one line so that
        // when reloadChat re-renders the text, pills appear together (not one
        // per paragraph), matching the immediate-send bubble layout.
        let payloadMsg = msg;
        for (const f of attachedFiles) {
        ...
        const _payloadImgs = attachedFiles.filter(f => f.type.startsWith('image/'));
        if (_payloadImgs.length > 0) {
            payloadMsg += '\n\n' + _payloadImgs.map(f => `[Attached Image: ${f.name}]`).join(' ');
        }
        const payload = { message: payloadMsg };
    ...
    try {
        ...
        // Inject edit/delete buttons into the just-created bubbles now that
        // the backend has saved the turn and we can look up their indices/msg_ids.
        // This replaces the old reloadChat() call — no full re-render needed.
        await _patchLatestBubbleButtons(assistantBubble);
    } finally {
        removeAttachedFile();
    ...
    try {
        ...
        if (res.ok) {
            ...
            if (data.indexing_triggered) {
                ...
                // Backend _bg_index thread is now running — check if we should show BUSY.
                startPassiveRebuildPoll();
            }
        } else {
        ...
        for (let i = 0; i < history.length; i++) {
            ...
            if (entry.role === "user" && !entry.silent) {
                ...
                if (typeof entry.content === 'string') {
                    ...
                    // Only embed an image URL if this isn't a video message.
                    // Video messages already have [Video frames: ...] in the text part
                    // which renders as the pill — embedding raw base64 here causes marked's
                    // lheading regex to catastrophically backtrack on huge single-line strings.
                    const isVideoMessage = contentToDisplay.includes('[Video frames:');
                    if (!isVideoMessage) {
                        // filter (not find) so ALL images are captured, not just the first
                        const imageParts = entry.content.filter(part => part.type === 'image_url');
                        for (const imagePart of imageParts) {
                            const imgUrl = imagePart.image_url.url;
                            if (imgUrl.startsWith('data:')) {
                                // Base64 data URLs cause lheading catastrophic backtrack in
                                // marked — collect all for direct DOM injection after render.
                                (_pendingImageUrl = _pendingImageUrl || []).push(imgUrl);
                            } else {
                                // Alt text intentionally blank — hides filename from user bubble
                                contentToDisplay += `\n\n![ ](${imgUrl})`;
                            }
                        }
                    }
                ...
                const _userBubble = appendMessage(currentLoadoutDisplayName, contentToDisplay, "user", userIndex, i, entry.msg_id || null);
                if (_pendingImageUrl) {
                    const msgContent = _userBubble.querySelector('.message-content');
                    if (msgContent) {
                        for (const _imgUrl of _pendingImageUrl) {
...
// ============================================================
// --- MEMORY PRESETS (World Info) ---
// ============================================================

let _activePresetId = null;  // tracks which pill is currently selected

async function loadMemoryPresets() {
    try {
        const res = await fetch('/get_memory_presets');
        const data = await res.json();
        renderMemoryPresetPills(data.presets || []);
    } catch (err) {
        console.error('Failed to load memory presets:', err);
    }
}

function renderMemoryPresetPills(presets) {
    const container = document.getElementById('memoryPresetPills');
    if (!container) return;
    container.innerHTML = '';
    if (!presets || presets.length === 0) {
        container.innerHTML = '<span class="memory-preset-empty">No presets yet. Type something and click ＋ Save Preset.</span>';
        return;
    }
    presets.forEach(preset => {
        const pill = document.createElement('div');
        pill.className = 'memory-preset-pill' + (preset.id === _activePresetId ? ' active' : '');
        pill.dataset.id = preset.id;
        pill.dataset.content = preset.content;
        pill.title = preset.content.length > 120 ? preset.content.slice(0, 120) + '…' : preset.content;

        const nameSpan = document.createElement('span');
        nameSpan.className = 'memory-preset-pill-name';
        nameSpan.textContent = preset.name;
        nameSpan.onclick = () => selectMemoryPreset(preset.id, preset.content, pill);

        const delBtn = document.createElement('span');
        delBtn.className = 'memory-preset-pill-del';
        delBtn.textContent = '×';
        delBtn.title = 'Delete preset';
        delBtn.onclick = (e) => {
            e.stopPropagation();
            deleteMemoryPreset(preset.id, preset.name);
        };
...
function selectMemoryPreset(id, content, pillEl) {
    // Deselect all pills
    document.querySelectorAll('.memory-preset-pill').forEach(p => p.classList.remove('active'));
    ...
    // Fill textarea
    const ta = document.getElementById('memoryInput');
    ta.value = content;
    ta.style.height = 'auto';
    ta.style.height = Math.min(ta.scrollHeight, 300) + 'px';
    // Update word count
    const words = content.trim().split(/\s+/).filter(Boolean).length;
    const wc = document.getElementById('memoryWordCount');
...
function openSavePresetForm() {
    const ta = document.getElementById('memoryInput');
    if (!ta.value.trim()) {
        showToast('Type some content first, then save it as a preset.', 'error');
        return;
    }
    const form = document.getElementById('memoryPresetSaveForm');
    form.style.display = 'flex';
    const nameInput = document.getElementById('memoryPresetNameInput');
    nameInput.value = '';
    nameInput.focus();
}

function cancelSavePreset() {
    document.getElementById('memoryPresetSaveForm').style.display = 'none';
    document.getElementById('memoryPresetNameInput').value = '';
}

async function confirmSavePreset() {
    const name = document.getElementById('memoryPresetNameInput').value.trim();
    ...
    try {
        const res = await fetch('/save_memory_preset', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
        ...
        if (res.ok) {
            ...
            cancelSavePreset();
            _activePresetId = data.preset?.id || null;
            await loadMemoryPresets();
            if (_activePresetId) {
                document.getElementById('updatePresetBtn').style.display = 'inline-block';
            }
        } else {
            showToast('Failed to save preset: ' + (data.error || 'Unknown error'), 'error');
        }
    } catch (err) {
...
async function updateSelectedPreset() {
    ...
    const content = document.getElementById('memoryInput').value.trim();
    if (!content) { showToast('Content is empty.', 'error'); return; }
    try {
        const res = await fetch('/update_memory_preset', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ id: _activePresetId, content })
        });
        const data = await res.json();
        if (res.ok) {
            ...
            const pill = document.querySelector(`.memory-preset-pill[data-id="${_activePresetId}"]`);
            if (pill) pill.classList.add('active');
        } else {
            showToast('Failed to update: ' + (data.error || 'Unknown error'), 'error');
        }
    } catch (err) {
        showToast('Network error updating preset.', 'error');
    }
}

async function deleteMemoryPreset(id, name) {
    try {
        const res = await fetch('/delete_memory_preset', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
// Shows/hides the batch flush size row based on the toggle state.
function _toggleBatchEvictionRow(enabled) {
    const row  = document.getElementById('batchEvictionSizeRow');
    const hint = document.getElementById('batchEvictionSizeHint');
...
async function openTokensModal() {
    ...
    try {
        ...
        // --- BATCH EVICTION ---
        const batchEnabled = tokens.batch_eviction_enabled === true;
        const batchSize    = tokens.batch_eviction_size ?? 0;
        document.getElementById('batchEvictionToggle').checked = batchEnabled;
        document.getElementById('batchEvictionSize').value    = batchSize;
        document.getElementById('batchEvictionSizeNum').value = batchSize;
        _toggleBatchEvictionRow(batchEnabled);
        // --- END BATCH EVICTION ---
...
async function saveTokenSettings() {
    ...
    const sharedTokens = {
        ...
        // --- BATCH EVICTION ---
        batch_eviction_enabled: document.getElementById('batchEvictionToggle').checked,
        batch_eviction_size: parseInt(document.getElementById('batchEvictionSizeNum').value) || 0,
        // --- END BATCH EVICTION ---
    };
...
async function _processIncomingFiles(files) {
    ...
    for (const file of toProcess) {
        ...
        if (file.size > 150 * 1024 * 1024 && !file.type.startsWith('video/')) {  // safe/conservative: 30MB
            showToast(`'${file.name}' is too large (max 150MB).`, 'error', 2500);
            continue;
        }
...
function getVideoDuration(videoFile) {
    return new Promise((resolve) => {
        ...
        video.addEventListener('error', () => { cleanup(); resolve(30); });
...
// them into OpenAI-style image_url content parts, which Ollama's /v1/chat/
// completions compat layer maps to its internal images[] array automatically.
// Resolution is capped at maxDim px on the longest side to keep payload sane.
//
// Fixes vs previous version:
//  • Blob URL is NO LONGER revoked here — removeAttachedFile() owns that lifecycle.
//  • Timestamps never start at exactly 0 (Chromium skips seeked for t=0).
//  • Per-seek watchdog timeout: stalled seeks skip the frame instead of hanging.
//  • numFrames is clamped to ≥1 so bad settings can't produce a 0-frame request.
// =============================================================================
async function extractVideoFrames(videoFile, numFrames = 8, jpegQuality = 0.82, maxDim = 512) {
    return new Promise((resolve, reject) => {
        // Guard: bad settings (floor=0, divisor huge) can still produce 0 here
        const safeNumFrames = Math.max(1, Math.floor(numFrames));
        ...
        // NOTE: blob URL lifecycle is owned by removeAttachedFile() — do NOT revoke here.
        video.src = videoFile.content;
        ...
        let frameIndex = 0;
        let timestamps = [];
        let seekTimer  = null;    // watchdog: fires if seeked never arrives
        let settled    = false;   // prevent double-resolve/reject

        // Clean up the hidden video element regardless of outcome
        function _cleanup() {
        ...
        video.addEventListener('error', () => {
            _reject(new Error(
                `Cannot load video "${videoFile.name}". ` +
                `The file may be corrupt, an unsupported codec, or the blob URL expired — ` +
        ...
        video.addEventListener('loadedmetadata', () => {
            ...
            if (!isFinite(duration) || duration <= 0) {
                _reject(new Error(`"${videoFile.name}" has no readable duration.`));
                return;
            }
            ...
            // Spread timestamps evenly across 2%–98% of the clip.
            // Starting at >0 avoids the Chromium "seeked never fires at t=0" bug.
            // Ending before 100% avoids end-of-stream decoder stalls.
            const tStart = Math.min(0.05, duration * 0.02);
            const tEnd   = duration * 0.98;

            if (safeNumFrames === 1) {
                timestamps = [(tStart + tEnd) / 2];
            } else {
                timestamps = Array.from({ length: safeNumFrames }, (_, i) =>
                    tStart + (i / (safeNumFrames - 1)) * (tEnd - tStart)
                );
            }
        ...
        video.addEventListener('seeked', () => {
            clearTimeout(seekTimer);
            // Mobile browsers (Chrome/Firefox on Android & iOS) fire 'seeked' before the
            // hardware decoder has actually written the pixel data into the video element.
            // Drawing immediately produces a black frame. Waiting two requestAnimationFrame
            // cycles gives the GPU compositor time to finish decoding and paint the frame
            // before the canvas reads it. Desktop browsers are unaffected — the extra ~2 rAF
            // delay is imperceptible (<32ms) and the watchdog is already cleared above.
        ...
        function seekNext() {
            if (frameIndex >= timestamps.length) {
                if (frames.length === 0) {
                    _reject(new Error(`No frames could be extracted from "${videoFile.name}".`));
                } else {
                    _resolve({ frames, timestamps: timestamps.slice(0, frames.length) });
                }
                return;
            }

            // Watchdog: if seeked doesn't fire within 6 s, skip this frame and continue.
            // This prevents a single bad keyframe from hanging the entire extraction.
            clearTimeout(seekTimer);
            seekTimer = setTimeout(() => {
                console.warn(
                    `[extractVideoFrames] Seek stalled at frame ${frameIndex} ` +
                    `(t=${timestamps[frameIndex]?.toFixed(2)}s) — skipping.`
                );
                frameIndex++;
                seekNext();
            }, 6000);
...
function removeAttachedFile(idx) {
    if (idx !== undefined) {
        // Revoke blob URL for video files before dropping the reference
        const f = attachedFiles[idx];
        if (f && f.type?.startsWith('video/') && f.content?.startsWith('blob:')) {
            URL.revokeObjectURL(f.content);
        }
        attachedFiles.splice(idx, 1);
    } else {
        // Legacy clear-all path (called after send or on error)
        attachedFiles.forEach(f => {
            if (f.type?.startsWith('video/') && f.content?.startsWith('blob:')) {
                URL.revokeObjectURL(f.content);
            }
        });
        attachedFiles = [];
...
function openSearchSettingsModal() {
    ...
    fetch('/get_search_settings')
        ...
        .then(settings => {
            ...
            // Scraping Domains
            const universalOn = settings.universal_scraping || false;
            document.getElementById('universalScrapingToggle').checked = universalOn;
            document.getElementById('scrapeableDomainsRow').style.opacity = universalOn ? '0.4' : '1';
            document.getElementById('scrapeableDomainsInput').value = (settings.scrapeable_domains || []).join(', ');
            document.getElementById('universalScrapingToggle').onchange = function() {
                document.getElementById('scrapeableDomainsRow').style.opacity = this.checked ? '0.4' : '1';
            };
...
async function saveSearchSettings() {
    ...
    const newSettings = {
        ...
        universal_scraping: document.getElementById('universalScrapingToggle').checked,
...
/**
 * Passive poll — call after any session switch or on page load.
 * Does ONE status check; if a rebuild is already running (background thread
 * started by session switch, model change, global toggle, etc.) it shows the
 * BUSY overlay and starts the full polling loop automatically.
 * If nothing is running it's a silent no-op.
 */
async function startPassiveRebuildPoll() {
    try {
        const res  = await fetch('/api/rebuild_status');
        if (!res.ok) return;
        const data = await res.json();
        if (data.running) {
            showBusyOverlay('Rebuilding index…', false);
            startPollingRebuildStatus();
        }
    } catch(e) { /* silent — network might not be ready yet */ }
}
...
window.addEventListener("DOMContentLoaded", async () => {
    ...
    // Check if a rebuild is already running on page load (e.g. page refresh mid-rebuild,
    // or a background thread started before the page was ready).
    startPassiveRebuildPoll();
    ...
    textarea.addEventListener("paste", function (e) {
        // ── File paste (images + real files from the file system) ──
        // Collect ALL kind='file' clipboard items in one pass.
        // The old code only matched image/* and returned early — any other file type
        // (pdf, txt, py, json…) fell through to the text handler and was silently dropped.
        ...
        if (clipItems) {
            ...
            if (rawFiles.length > 0) {
                ...
                const namedFiles = rawFiles.map(({ item, file }) => {
                    if (item.type.startsWith('image/')) {
                        // Pasted images have no real filename — generate one with a timestamp
                        const ext = item.type.split('/')[1] || 'png';
                        return new File([file], `pasted_image_${stamp}.${ext}`, { type: item.type });
                    }
                    // Real files copied from the file system already carry their filename
                    return file;
                });
                _processIncomingFiles(namedFiles);
                return;
            }
        }
    ...
    setupSyncedSlider('batchEvictionSize',  'batchEvictionSizeNum');

    // Batch eviction toggle — show/hide the flush size row
    document.getElementById('batchEvictionToggle').addEventListener('change', function() {
        _toggleBatchEvictionRow(this.checked);
    });
...
async function _idleSend() {
    ...
    // silent:true tells the backend to mark the user payload as silent so
    // reloadChat() and page-refresh never surface it as a visible user bubble.
    const fetchPromise = fetch('/chat_stream', {
        method:  'POST',
        headers: { 'Content-Type': 'application/json' },
        body:    JSON.stringify({ message: idlePrompt, silent: true }),
        signal
    });
...
const VIDEO_DEFAULTS = {
    frame_divisor:   2,     // safe/conservative: 3
    frame_floor:     8,     // safe/conservative: 4
    frame_cap:       200,   // safe/conservative: 60
    jpeg_quality:    0.92,  // safe/conservative: 0.82
    resolution:      720,   // safe/conservative: 512
};
...
// (Blob retention toggle removed — vanilla mode only)
