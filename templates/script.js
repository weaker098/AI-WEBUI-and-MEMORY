...
function formatMarkdown(text, streamComplete = false) {
    ...
    // temporarily append a closing ``` so marked sees a complete block this render pass.
    ...
    // 0.2. EARLY STRIP OF RESULT BLOCKS — runs before ANY think-tag processing.
    // Search/Recall result injections can contain <think>...</think> tags from past AI sessions
    // stored in FAISS memory. If those nested tags reach the auto-close (step 0.5) or the
    // thinkRegex (step 1), they consume the closing </think> of the *next real* think block,
    // leaving it naked in the UI. Stripping results here quarantines the problem at the source.
    // The full strip runs again later (after tool-widget replacement) — this early pass is a
    // safety net specifically for the think-tag conflict.
    if (searchResultHeader || recallResultHeader) {
        const _earlyHeaders = [searchResultHeader, recallResultHeader].filter(Boolean);
        _earlyHeaders.forEach(header => {
            const _esc = header.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
            const _earlyStrip = new RegExp(`${_esc}[\\s\\S]*?(?:\\[\\/End.*?Results\\]|$)`, 'gi');
            processedText = processedText.replace(_earlyStrip, '');
        });
    }
    ...
    // 1d. STRIP SEARCH/RECALL RESULT BLOCKS (INVISIBILITY CLOAK) — runs BEFORE thinkRegex.
    // CRITICAL ORDER: must happen before step 1 (thinkRegex) so that any <think>/<\/think> tags
    // embedded inside recalled memory blocks (e.g. past AI thinking stored in FAISS) are removed
    // BEFORE the thinkRegex scans the text. Without this, a nested <think> inside [Recall Results]
    // steals the closing </think> from the *next real* think block, leaving it "naked" in the UI.
...
async function send() {
    ...
    if (!msg && !attachedFile && !_idleFiring) return;
    ...
    if (attachedFile && attachedFile.type.startsWith('image/')) {
        // If the image was already saved to the server via /save_image, send the
        // filename so the backend can read from disk — avoids sending the full
        // base64 blob twice (once to /save_image, again here). Falls back to
        // inline base64 if the pre-upload failed and serverFilename is null.
        const imagePayload = attachedFile.serverFilename
            ? { prompt: msg, server_filename: attachedFile.serverFilename }
            : { prompt: msg, image_data: attachedFile.content };
...
async function deleteAttachedFile(index, msgId) {
    openConfirmationModal('Remove this file from context memory?', async () => {
        try {
            ...
            const data = await res.json();
            if (res.ok) {
                const item = document.getElementById(`file-mem-item-${index}`);
                if (item) item.remove();
...
function _wrapTextAsFile(text) {
    ...
    attachedFile = { name: filename, content: text, type: 'text/plain', serverFilename: null };
    sessionStorage.setItem('attachedFile', JSON.stringify(attachedFile));
    displayFilePreview();
...
send = async function() {
    if (isSending) {
        ...
        _msgQueue.push({ msg, file: attachedFile ? Object.assign({}, attachedFile) : null });
