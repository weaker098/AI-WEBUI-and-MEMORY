...
let _koboldCache     = { endpoint: 'http://127.0.0.1:5001/v1/chat/completions' };
...
async function saveTokenSettings() {
    if (_isOpenRouter()) {
        ...
        try {
            ...
            const result = await res.json();
            if (res.ok) { closeTokensModal(); showToast('OpenRouter max tokens saved', 'success'); }
            else showToast("Error: " + (result.error || "Unknown"), 'error');
...
async function openAppSettingsModal() {
    ...
    try {
        ...
        document.getElementById('visionEnabledToggle').checked     = settings.vision_enabled    || false;
...
async function saveAppSettings() {
    ...
    const newSettings = {
        ...
        ...(mode === 'kobold'
            ? { kobold_endpoint: endpoint,
                llm_api_endpoint: endpoint, llm_model_name: '', llm_api_key: '' }
...
async function loadAppearanceAndNameSettings() {
    ...
    try {
        ...
        if (response.ok) {
            ...
            const userNameInput = document.getElementById('userNameInput');
            if (userNameInput) userNameInput.value = names.user_name || 'User';
        }
    } catch(e) {}
