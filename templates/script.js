...
let _generationId = 0;  // incremented each send() — stale finally/reloadChat checks against this
...
if (chatlog && scrollBtn) {
    // Passive: just show/hide the arrow based on scroll position. No fighting, no timers.
...
// Arrow button: one-shot manual jump to bottom. No auto-scroll during streaming.
...
async function copyCode(button) {
    ...
    try {
        ...
        // The old approach required a hidden textarea, a DOM insert, and execCommand —
        // all of which are either removed or being removed from modern browsers.
        ...
        while (true) {
            ...
            if (streamingSettings.streamingDelayEnabled) {
                for (const char of chunk) {
                    ...
                    // Bail immediately if aborted — stops the ghost typewriter effect
...
async function abortGeneration() {
    // 1. Poison the current generation — stale finally/reloadChat in send() check this.
    _generationId++;
    const abortedGenId = _generationId;  // capture so the deferred reload can self-cancel

    // 2. Kill the frontend stream reader immediately
    if (_streamAbortController) {
        _streamAbortController.abort();
        _streamAbortController = null;
    }
    // 3. Safety-net UI reset — send()'s finally is gated by genId so won't fire for
    //    this generation. Reset button manually here instead.
    isSending = false;
    ...
    // 4. Tell KoboldCPP backend to stop too (no-op for OpenRouter)
    try {
    ...
    // 5. Deferred reloadChat — snaps the DOM to the saved truncated state.
    //    Short delay lets the backend finish writing the truncated message.
    //    Self-cancels if the user already sent a new message (genId moved on).
    setTimeout(async () => {
        if (_generationId === abortedGenId) {
            await reloadChat();
        }
    }, 400);
}
...
async function send() {
    ...
    isSending = true;
    const myGenId = ++_generationId;  // this send owns this generation ID
    ...
    try {
        ...
        // Only reset state if we're still the active generation.
        // abortGeneration() increments _generationId, so stale finally blocks skip.
    ...
    // Gated — skip if abort fired and a new send already started.
    if (_generationId === myGenId) {
        await reloadChat();
    }
}
    ...
    try {
        ...
        for (let i = 0; i < history.length; i++) {
            ...
            if (entry.role === "user" && !entry.silent) {
                ...
                appendMessage(currentUserName, contentToDisplay, "user", userIndex, i, entry.msg_id || null);
                userIndex++;
...
function openConfirmationModal(message, actionCallback, cancelCallback = null) {
    ...
    confirmBtn.onclick = async () => {
        pendingCancelAction = null;  // FIX: prevent cancel callback from firing on confirm
...
window.addEventListener("DOMContentLoaded", async () => {
    ...
    try {
        ...
        if (_nameRes.ok) {
            ...
            if (_nameData.user_name)      currentUserName = _nameData.user_name;
        }
