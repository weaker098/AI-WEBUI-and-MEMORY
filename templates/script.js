...
function avatarContent(avatarVal) {
    ...
    if (avatarVal.startsWith('data:image') || avatarVal.startsWith('/') || avatarVal.startsWith('http')) {
        return `<img src="${avatarVal}" alt="avatar" style="width:100%;height:100%;object-fit:cover;border-radius:50%;">`;
    }
...
if (chatlog) {
    chatlog.addEventListener("click", (e) => {
        // JS-BUG-6 FIX: copy button uses data-action delegation instead of inline onclick
...
if (chatlog && scrollBtn) {
    chatlog.addEventListener("scroll", () => {
        // Ignore scroll events we triggered ourselves (auto-scroll)
        if (isProgrammaticScroll) return;

        // Any real user scroll → pause auto-scroll immediately
        userScrolledUp = true;
        scrollBtn.style.display = "flex";

        // Reset the 10 second resume timer on every user scroll
        if (userScrollTimer) clearTimeout(userScrollTimer);
        userScrollTimer = setTimeout(() => {
            userScrolledUp = false;
            const distanceToBottom = chatlog.scrollHeight - chatlog.scrollTop - chatlog.clientHeight;
            if (distanceToBottom <= 200) scrollBtn.style.display = "none";
        }, 3500);
    });
}

// User clicked the arrow — resume auto-scroll immediately
...
// Use this for all auto-scrolls during streaming — won't trigger user scroll detection
function programAutoScroll(el) {
...
function copyCode(button) {
    ...
    // The code is now inside a <pre><code> block
...
function formatMarkdown(text) {
        ...
        return `
            <div class="thought-container">
                <div class="thought-header">
                    <span>🧠 Thoughts...</span>
                    <span class="click-hint">(click to peek)</span>
                </div>
    ...
    return cleanHtml.replace(codeBlockRegex, (match) => {
        return `
...
function appendMessage(name, text, cls, index = null, memIndex = null) {
    ...
    const editBtn = memIndex !== null
        ...
        : "";
    const deleteBtn = cls === "user" && index !== null
        ? `<button class="msg-action-btn delete" onclick="deletePair(${index})" title="Delete">🗑</button>`
        : "";
...
async function streamAssistantResponse(fetchPromise, assistantBubble) {
    const chatlog = document.getElementById("chatlog");
    ...
    function _renderAndRestoreStates(html) {
        ...
        contentSpan.querySelectorAll('.thought-container').forEach((el, i) => {
            if (savedStates[i]) el.classList.add('expanded');
    ...
    try {
        ...
        while (true) {
            ...
            if (streamingSettings.streamingDelayEnabled) {
                ...
                // Raw TPS mode — no delay, instant chunk render, max speed.
                fullResponse += chunk;
                _renderAndRestoreStates(formatMarkdown(fullResponse));
                programAutoScroll(chatlog);
            }
...
async function abortGeneration() {
    // 1. Kill the frontend stream reader immediately
    if (_streamAbortController) {
        _streamAbortController.abort();
        _streamAbortController = null;
    }
    // 2. Tell KoboldCPP backend to stop too
    try {
...
async function send() {
    ...
    if (displayMsg.trim()) {
        appendMessage(currentUserName, displayMsg.trim(), "user");
    }
    ...
    await streamAssistantResponse(fetchPromise, assistantBubble);
    ...
    // Reload chat so all messages get correct indexes and delete buttons appear
    await reloadChat();
}
...
async function newSession() {
    ...
    try {
        ...
        const data = await res.json();
        const newId = data.session_id || data.message.split(': ')[1];
    ...
    try {
        ...
        for (let i = 0; i < history.length; i++) {
            ...
            if (entry.role === "user" && !entry.silent) {
                ...
                appendMessage(currentUserName, contentToDisplay, "user", userIndex, i);
                userIndex++;
                ...
                if (hasToolCall && i + 1 < history.length) {
                    ...
                    while (scanIdx < history.length) {
                        ...
                        if (scanEntry.role === 'system') {
                            // Tool result block — always absorb regardless of silent flag.
                            // silent=true means "don't render standalone", NOT "skip in merge".
                            // Persisted search results are saved with silent:true and MUST
                            // be absorbed here or they split into two broken bubbles.
                            mergedContent += '\n\n' + scanEntry.content;
                            ...
                            // Another tool-call assistant turn → absorb and keep scanning
                            // Final assistant turn (no tool) → absorb and stop
                            mergedContent += '\n\n' + scanEntry.content;
                            skipIndices.add(scanIdx);
                            scanIdx++;
                            if (!scanHasTool) break; // final turn reached
                        } else {
                            // Anything else (user msg, pinned system, etc.) → stop
                            break;
                        }
                    }

                    appendMessage(assistantName, mergedContent, "grok", null, i);
                    continue;
                }

                // Normal assistant message (no merge needed)
                appendMessage(assistantName, entry.content, "grok", null, i);
            }
        }
        isProgrammaticScroll = true; chatlog.scrollTop = chatlog.scrollHeight; requestAnimationFrame(() => { isProgrammaticScroll = false; });
        await updateAllStatusUI();
...
async function deletePair(index) {
    try {
        ...
        const res = await fetch("/delete_message", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ index, session_id: sessionId })
        });
        const data = await res.json();
...
async function setSystemPrompt() {
    ...
    // Allow saving empty prompt (e.g. for a brand new persona) — just confirm
    if (!systemPromptText) {
        const ok = confirm("The prompt is empty. Save anyway? The persona will have a blank system prompt.");
        if (!ok) return;
    }
    ...
    try {
        ...
        if (searchData.results && searchData.results.length > 0) {
            searchData.results.forEach(result => {
                ...
                item.id = `search-item-${result.index}`;
                ...
                deleteBtn.textContent = 'Delete';
                deleteBtn.className = 'action-btn delete-btn';
                deleteBtn.onclick = () => deleteSearchResult(result.index);
...
async function deleteSearchResult(index) {
...
async function saveEditedVisibleIntro() {
    ...
    if (!newContent) {
        if (!confirm("This is empty. Do you want to delete this intro instead?")) {
            return;
        }
...
async function deleteSession(sessionId) {
    ...
    return new Promise(resolve => {
        ...
        const origClose = closeConfirmationModal;
        closeConfirmationModal = function() {
            origClose();
            closeConfirmationModal = origClose;
            resolve(false);
        };
    });
}
...
async function deleteSelectedPersona() {
    ...
    // Use the existing confirmationModal
    const msgEl = document.getElementById('confirmationMessage');
    const confirmBtn = document.getElementById('confirmActionBtn');
    if (msgEl) msgEl.textContent = `Delete persona "${personaToDelete}"? This cannot be undone.`;
    if (confirmBtn) confirmBtn.textContent = 'Yes, Delete';

    // Show confirmation modal on top of persona modal
...
async function saveTemperatures() {
    const newTemps = {
        chat_stream: document.getElementById('chatStreamTemp').value,
        summarization: document.getElementById('summarizationTemp').value,
    };
    ...
    try {
        ...
        const result = await response.json();
        if (response.ok) {
            console.log("Temperatures saved:", result.message);
            closeTemperaturesModal();
            showToast('Temperatures saved', 'success');
        } else {
            showToast("Error saving temperatures: " + (result.error || "Unknown error"), 'error');
        }
    } catch (error) {
...
async function openAppSettingsModal() {
    ...
    try {
        ...
        document.getElementById('llmApiEndpoint').value = settings.llm_api_endpoint || '';
...
async function saveAppSettings() {
    const newSettings = {
        llm_api_endpoint: document.getElementById('llmApiEndpoint').value.trim(),
    ...
    try {
        ...
        if (response.ok) {
            ...
            showToast("Application settings saved. Some changes may require a page refresh.", 'success');
        } else {
...
async function loadApiProfiles() {
    try {
        ...
        _updateApiProfileActiveBadge(data.active);
...
async function switchApiProfile() {
    ...
    try {
        ...
        const data = await res.json();
        if (res.ok) {
            // Apply profile values into the live fields immediately
            document.getElementById('llmApiEndpoint').value = data.profile.endpoint || '';
            document.getElementById('llmModelName').value   = data.profile.model    || '';
            document.getElementById('llmApiKey').value      = data.profile.api_key  || '';
...
function openResonanceSettingsModal() {
    ...
    // Check master resonance toggle state before rendering settings
    fetch('/get_tokens')
        .then(r => r.json())
        .then(tokens => applyResonanceMasterState(tokens.resonance_enabled !== false))
        .catch(() => applyResonanceMasterState(true)); // fail-open
    fetch('/get_resonance_settings')
        .then(response => response.json())
        .then(settings => {
            ...
            loadRerankerSettings();
            
            // --- Load Stage 3 Sanity Check Settings ---
            loadSanityCheckSettings();
        })
        .catch(error => {
...
async function saveResonanceSettings() {
    const allSettings = {
        recalled_message_char_limit: parseInt(document.getElementById('recalledMessageCharLimit').value),
        ...
        raw_rag_score_threshold: parseFloat(document.getElementById('rawRagThreshold').value),
        raw_rag_max_recall: parseInt(document.getElementById('rawRagMaxRecall').value, 10),
    ...
    try {
        ...
        if (res.ok) {
            console.log("Resonance settings saved:", r.message);
            
            // Save reranker settings
            const rerankerSaved = await saveRerankerSettings();
            
            // Save Stage 3 sanity check settings
            const sanitySaved = await saveSanityCheckSettings();
            
            if (rerankerSaved && sanitySaved) {
                closeResonanceSettingsModal();
            showToast('Memory settings saved', 'success');
            }
        } else {
            showToast("Error saving settings: " + (r.error || "Unknown error"), 'error');
        }
    } catch (error) {
        console.error("Network error saving resonance settings:", error);
        showToast("Network error saving resonance settings.", 'error');
    }
}
...
async function applyActiveUserLoadoutAvatar() {
    try {
        ...
        const data = await res.json();
        currentUserName = data.display_name || 'User';
        if (data.avatar_url) {
    ...
    try {
        ...
        if (res.ok) {
            ...
            if (activeInfo.loadout_name === name) {
                await fetch('/switch_user_loadout', {
                    method: 'POST',
                    headers: {'Content-Type': 'application/json'},
                    body: JSON.stringify({loadout_name: name})
                });
...
function displayFilePreview() {
    ...
    container.innerHTML = `
        <div class="file-pill">
            ${previewContent}
            <span class="file-pill-name">${attachedFile.name}</span>
...
function openFaissSettingsModal() {
    ...
    fetch('/get_faiss_settings')
        ...
        .then(settings => {
            ...
            document.getElementById('globalMemoryLimit').value = settings.global_memory_session_limit || 3;
...
function updateCtxSliderVisibility(modelChoice, ctxLength) {
    ...
    if (HIGH_CTX_MODELS.includes(modelChoice)) {
        ...
        const val = ctxLength && ctxLength > 0 ? ctxLength : 2048;        slider.value = val;
...
async function saveFaissSettings() {
    ...
    const modelChanged  = selectedModel  !== (saveFaissSettings._lastModel  || selectedModel);
    const metricChanged = selectedMetric !== (saveFaissSettings._lastMetric || selectedMetric);
    const ASYMMETRIC_MODELS_JS = ['nomic', 'jina-small', 'jina-base'];
    const prefixChanged = _prefixModeDraft !== (saveFaissSettings._lastPrefixMode || _prefixModeDraft);
    const prefixIsStructural = prefixChanged && ASYMMETRIC_MODELS_JS.includes(selectedModel);
    const looksHeavy = modelChanged || metricChanged || prefixIsStructural;
    ...
    const newSettings = {
        ...
        global_memory_session_limit: parseInt(document.getElementById('globalMemoryLimit').value),
    ...
    try {
        ...
        if (response.ok) {
            ...
            // Use backend's authoritative structural_change flag for success msg
            ...
            bannerMsg.textContent  = wasStructural
                ...
                : 'Settings saved. No rebuild needed.';
...
async function deleteVisibleIntro(index) {
    // Call our fancy modal instead of the browser alert
    openConfirmationModal(
...
function openConfirmationModal(message, actionCallback) {
    ...
    // Store the action to run if they click "Yes"
    pendingConfirmationAction = actionCallback;

    // Set up the "Yes" button one-time listener
    confirmBtn.onclick = async () => {
...
function openInlinePrompt({ title, subtitle = '', hint = '', defaultValue = '', placeholder = '' }) {
    ...
    return new Promise((resolve) => {
        ...
        input.onkeydown = (e) => {
            ...
            if (e.key === 'Escape') { closeInlinePrompt(); resolve(null); }
        };
...
async function clearGlobalMemory() {
    openConfirmationModal(
        "Clear all global memory vectors? This cannot be undone — the index will rebuild automatically on next message.",
        async () => {
            try {
                const res = await fetch("/clear_global_memory", { method: "POST" });
                const data = await res.json();
                if (res.ok) {
                    appendMessage("System", `🗑️ ${data.message}`, "system-notification");
                } else {
                    showToast("Error: " + (data.error || "Unknown error"), 'error');
                }
            } catch (err) {
                console.error("Error clearing global memory:", err);
                showToast("Network error clearing global memory.", 'error');
            }
        }
    );
}
...
window.addEventListener("DOMContentLoaded", async () => {
    ...
    await loadStreamingSettings();  // FIX BUG 7: was unawaited — settings must be loaded
    // before send() is possible, but await is the correct contract for async functions.
    ...
    setupSyncedSlider('globalMemoryLimit',  'globalMemoryLimitNum');
...
function openChunkingModal() {
    ...
    fetch('/get_chunking_settings')
        ...
        .then(s => {
            ...
            document.getElementById('longContextThreshold').value =
                s.long_context_threshold ?? 2;
...
async function saveChunkingSettings() {
    ...
    const payload = {
        ...
        long_context_threshold:         parseInt(document.getElementById('longContextThreshold').value),
...
async function saveRerankerSettings() {
     ...
     */
    const settings = {
        reranker_enabled: document.getElementById('rerankerEnabledToggle').checked,
        reranker_model: document.getElementById('rerankerModelSelect').value,
        reranker_expansion_factor: parseInt(document.getElementById('rerankerExpansionFactor').value),
        reranker_ctx_length: parseInt(document.getElementById('rerankerCtxSlider').value) || null,
        reranker_score_threshold: parseFloat(document.getElementById('rerankerScoreThreshold').value),
        reranker_batch_size: parseInt(document.getElementById('rerankerBatchSize').value)
    };
...
function loadSanityCheckSettings() {
    fetch('/get_sanity_check_settings')
        .then(response => response.json())
        .then(settings => {
...
async function saveSanityCheckSettings() {
    const settings = {
        sanity_check_enabled: document.getElementById('sanityCheckEnabledToggle').checked,
        sanity_check_intent_enabled: document.getElementById('sanityIntentEnabledToggle').checked,
        sanity_check_intent_threshold: parseFloat(document.getElementById('sanityIntentThreshold').value),
        sanity_check_content_threshold: parseFloat(document.getElementById('sanityContentThreshold').value),
        ...
        sanity_check_buffer_multiplier: parseFloat(document.getElementById('sanityBufferMultiplier').value),
        sanity_check_show_scores: document.getElementById('sanityShowScoresToggle').checked,
        // --- Zeroshot DLC ---
        ...
        zeroshot_ctx_length: parseInt(document.getElementById('zeroshotCtxLength')?.value || 512),
        zeroshot_threshold_enabled: document.getElementById('zeroshotThresholdEnabled')?.checked ?? true,
        zeroshot_entailment_threshold: parseFloat(document.getElementById('zeroshotThreshold')?.value || 0.70),
        ...
        // --- Advanced NLI ---
        zeroshot_score_floor: parseFloat(document.getElementById('zsScoreFloor')?.value ?? 0.10),
        zeroshot_top_n_hypotheses: parseInt(document.getElementById('zsTopN')?.value ?? 0),
    };
...
async function saveAvatarCrop() {
    ...
    try {
        ...
        if (avatarType === 'persona') {
            ...
            _updateUserLoadoutAvatarPreview(loadout_name, loData.loadouts[loadout_name] || {});
            await applyActiveUserLoadoutAvatar();
        }
        await reloadChat();
...
function _updateZsPreview() {
    ...
    preview.innerHTML = `
        threshold: <span style="color:#7ec87e">${thresh}</span> &nbsp;|&nbsp;
        aggregation: <span style="color:#7ec87e">${agg}</span><br>
        score_floor: <span style="color:#7ec87e">${floor}</span> &nbsp;|&nbsp;
        top_n: <span style="color:#7ec87e">${topN === '0' || topN === 0 ? 'all' : topN}</span>
    `;
}
