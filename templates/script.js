...
async function streamAssistantResponse(fetchPromise, assistantBubble, signal = null) {
    ...
    function _renderAndRestoreStates(html) {
        ...
        // Save horizontal scroll positions of all existing <pre> blocks by index.
        ...
        // Split html on code-container boundaries so we can compare containers individually.
        // The split captures the dividers so we can reconstruct ordering.
        const CODE_OPEN  = '<div class="code-container">';
        const CODE_CLOSE = '</div>'; // we scan for the matching close below

        // Extract all .code-container blocks from the new HTML along with their positions.
        // Returns array of { html: string, start: number, end: number }
        function _extractCodeContainers(src) {
            const blocks = [];
            let searchFrom = 0;
            while (true) {
                const openIdx = src.indexOf(CODE_OPEN, searchFrom);
                if (openIdx === -1) break;
                // Walk forward counting open/close div tags to find the matching closer
                let depth = 0;
                let i = openIdx;
                while (i < src.length) {
                    if (src.startsWith('<div', i)) depth++;
                        ...
                        depth--;
                        if (depth === 0) {
                            const closeIdx = i + '</div>'.length;
                            blocks.push({ html: src.slice(openIdx, closeIdx), start: openIdx, end: closeIdx });
                            searchFrom = closeIdx;
                            break;
                        }
                    }
        ...
        if (newContainers.length === 0 || newContainers.length !== liveContainers.length) {
            // Container count changed (new block appeared, or none yet) — full rebuild this frame.
            contentSpan.innerHTML = html;
        } else {
            // Same number of containers — do a per-container diff.
            // 1. Rebuild everything EXCEPT the code containers (prose, thought boxes, etc.)
            //    by replacing each container's HTML with a stable sentinel placeholder.
            let prosePart = html;
            newContainers.forEach((blk, idx) => {
                prosePart = prosePart.replace(blk.html, `<div class="code-container-sentinel" data-idx="${idx}"></div>`);
            });

            // Create a temp element to parse just the prose HTML
            const tmp = document.createElement('div');
            tmp.innerHTML = prosePart;

            // Swap each sentinel back with the live (or updated) container
            tmp.querySelectorAll('.code-container-sentinel').forEach((sentinel, idx) => {
                const liveContainer = liveContainers[idx];
                const newContainerHtml = newContainers[idx].html;

                if (liveContainer) {
                    // Only update the <code> element inside the container if the text changed.
                    // This keeps the outer shell (.code-container border, copy-btn, lang label)
                    // completely untouched — no flicker.
                    const liveCode = liveContainer.querySelector('code');
                    const tmpHolder = document.createElement('div');
                    tmpHolder.innerHTML = newContainerHtml;
                    const newCode = tmpHolder.querySelector('code');

                    if (liveCode && newCode && liveCode.innerHTML !== newCode.innerHTML) {
                        liveCode.innerHTML = newCode.innerHTML;
                    }
                    sentinel.replaceWith(liveContainer);
                } else {
                    // No live container for this slot — insert the new one
                    const tmpHolder = document.createElement('div');
                    tmpHolder.innerHTML = newContainerHtml;
                    sentinel.replaceWith(tmpHolder.firstChild);
                }
            });

            // Now set the prose (non-code) parts of contentSpan.
            // We do this by adopting all of tmp's children into contentSpan.
            // But we need to preserve the live code containers already placed above.
            // Strategy: collect children of tmp (which now has the live containers re-inserted),
            // then replace contentSpan's children wholesale.
            const newChildren = [...tmp.childNodes];
            // Clear contentSpan without removing the nodes we care about
            while (contentSpan.firstChild) contentSpan.removeChild(contentSpan.firstChild);
            newChildren.forEach(child => contentSpan.appendChild(child));
        }
        ...
        // Restore code block horizontal scroll positions via rAF (layout hasn't run yet here).
