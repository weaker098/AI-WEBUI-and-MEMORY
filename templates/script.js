...
// --- Scroll tracking ---
// Simplified: no auto-scroll during streaming. Arrow button = manual one-shot only.
// Arrow visibility is driven purely by passive scroll position checks.
...
if (chatlog && scrollBtn) {
    // Passive: only show/hide arrow based on scroll position. No fighting, no timers.
...
// Stub — streaming no longer auto-scrolls. Kept so any missed callsite fails silently.
...
async function abortGeneration() {
    ...
    // 2. Safety-net UI reset — the finally block in send() also does this, but if the
    //    abort fires before the stream reader is entered the finally may not execute,
    //    leaving the button permanently stuck as a Stop button.
    isSending = false;
    ...
    // 3. Tell KoboldCPP backend to stop too (no-op for OpenRouter — server handles that gracefully)
    try {
...
async function send() {
    ...
    try {
        ...
        // Clear the abort controller reference now that the stream is done
        _streamAbortController = null;
    }
