...
if (chatlog) {
    ...
    chatlog.addEventListener("pointerdown", (e) => {
        ...
        if (!container) return;
        if (container.classList.contains('thought-failed')) return; // locked — failed think block
...
window.addEventListener("DOMContentLoaded", async () => {
    ...
    textarea.addEventListener("input", function () {
        ...
        // Manual-typing overflow fallback (paste events handled smarter below)
        if (pasteToFileLength > 0 && this.value.length >= pasteToFileLength) {
            _wrapTextAsFile(this.value);
            this.value = '';
            this.style.height = 'auto';
        }
    });
...
async function saveZeroshotAdvanced() {
    ...
    // FIX: Always include the parent-modal fields (threshold, aggregation, enabled
    // flags) alongside the advanced-only fields, so a partial POST can never
    // silently wipe settings if the backend endpoint is ever changed to do a
    // full replace instead of a dict.update() merge.
    ...
    const threshold = parseFloat(document.getElementById('zeroshotThreshold')?.value ?? 0.70);
    const aggregation = document.querySelector('input[name="zeroshotAggregation"]:checked')?.value ?? 'max';
    const thresholdEnabled = document.getElementById('zeroshotThresholdEnabled')?.checked ?? true;
    const zsEnabled = document.getElementById('zeroshotIntentToggle')?.checked ?? false;
    const sanityEnabled = document.getElementById('sanityCheckEnabledToggle')?.checked ?? false;
