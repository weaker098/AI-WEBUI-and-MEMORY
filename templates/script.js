...
function formatMarkdown(text, streamComplete = false) {
    ...
    // ^superscript^  →  <sup>
    // ~subscript~    →  <sub>
    // NOTE: *word* italic is now handled natively by marked (html: true is set).
    //       The old manual regex *word* → <em> was redundant and silently broken —
    //       marked was escaping the injected <em> tags right after. Removed.
    // ALL run BEFORE code-block restore so they never fire inside fenced code blocks.
    // (Code blocks are still placeholders at this point — content is fully protected.)
