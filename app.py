...
def load_video_settings():
    ...
    defaults = {
        "frame_divisor": 3,     # 1 frame per N seconds of video
        "frame_floor":   4,     # minimum frames regardless of clip length
        "frame_cap":     60,    # server-side hard cap (safety net above frontend cap)
        "jpeg_quality":  0.82,  # JPEG compression quality sent from frontend (0.1–1.0)
        "resolution":    512,   # MAX_DIM — longest edge cap in pixels
    }
...
def set_video_settings():
    ...
    try:
        ...
        if "frame_cap" in data:
            data["frame_cap"] = max(4, min(120, int(data["frame_cap"])))
