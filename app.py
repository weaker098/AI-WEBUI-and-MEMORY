...
def api_set_system_prompt():
    ...
    if isinstance(persona_entry, dict) and persona_entry.get("is_character_card") and make_active:
        live_user  = NAME_SETTINGS.get("user_name", "User") or "User"
...
def import_character_card():
    ...
    user_name = NAME_SETTINGS.get("user_name", "User") or "User"
    description           = _resolve_card_templates(description,           nickname, user_name)
    personality           = _resolve_card_templates(personality,           nickname, user_name)
    scenario              = _resolve_card_templates(scenario,              nickname, user_name)
    first_mes             = _resolve_card_templates(first_mes,             nickname, user_name)
    system_prompt_field   = _resolve_card_templates(system_prompt_field,   nickname, user_name)
    post_history_instruct = _resolve_card_templates(post_history_instruct, nickname, user_name)
    alt_greetings         = [_resolve_card_templates(g, nickname, user_name) for g in alt_greetings]
