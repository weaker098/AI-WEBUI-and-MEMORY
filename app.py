        ...
        if RESONANCE_SETTINGS.get("chunking_enabled", False) and \
           RESONANCE_SETTINGS.get("chunk_retrieval_mode", "precision") == "long_context":
            ...
            if RESONANCE_SETTINGS.get("paired_memory_enabled", False) and reformed_messages:
                ...
                # Build msg_id → memory index map for fast neighbour lookup
                mid_to_mem_idx = {}
                for _i, _m in enumerate(memory):
                    _mid = _m.get("msg_id")
                ...
                for r_msg in reformed_messages:
                    ...
                    if mem_idx is None:
                        # Can't locate in memory — inject the reformed message alone
                        _add_pair(r_msg)
                        continue
    ...
    for key, value in data.items():
        if key in TOKEN_SETTINGS:
            try:
                ...
                # NOTE: precision_mode_enabled is intentionally excluded here — it is an
                # orphaned dead flag. Lane/precision routing is controlled exclusively by
                # RESONANCE_SETTINGS["chunk_retrieval_mode"]. See TOKEN_SETTINGS defaults.
                if key in ["resonance_enabled", "coding_mode_enabled", "ghost_memory_enabled"]:
