...
def chat_stream():
    ...
    def _agent_response_generator_inner(_last_acc, _is_saved):
        ...
        while current_turn < max_agent_turns:
            ...
            # --- ALWAYS INDEX: Index user + assistant together at end of turn ---
            # User is indexed here (not at arrival) so intent/recall/LLM all run first.
            # Assistant is indexed right after so both land in one clean sequence.
            if RESONANCE_SETTINGS.get("always_index_messages", False):
                try:
                    if user_payload and user_payload.get("content"):
