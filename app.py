...
def load_video_settings():
    ...
    defaults = {
        ...
        "keep_image_blobs":  False, # False = ephemeral (strip b64 from memory); True = vanilla (keep)
    }
...
# =============================================================================
# --- EPHEMERAL BLOB HELPER ---
# Replaces base64 image_url blobs with a lightweight server path (if one is
# available) so the image stays visible in the UI on reload, while the fat
# blob is never written to the history JSON.
# keep_image_blobs=False (default): call this before save_memory().
# keep_image_blobs=True  (vanilla):  skip — full blobs stay in history.
# =============================================================================
def _strip_blobs_for_memory(content, server_url=None):
    """
    Given message content (str or list of parts):
      - Strings are returned as-is.
      - image_url parts whose URL is a data: blob are replaced with server_url
        when provided, or dropped when no server reference is available.
      - Text parts are always preserved.

    `server_url` should be a relative path the frontend can render, e.g.
    '/get_image/abc123.png'.  Pass None when no server file exists.
    """
    ...
    result = []
    server_url_used = False
    for part in content:
        if part.get("type") == "image_url":
            url = part.get("image_url", {}).get("url", "")
            if url.startswith("data:"):
                if server_url and not server_url_used:
                    # Swap the fat blob for the slim server path —
                    # image stays renderable in the UI on reload.
                    result.append({"type": "image_url", "image_url": {"url": server_url}})
                    server_url_used = True
                # If no server_url or already used, drop this blob entirely.
            else:
                result.append(part)  # already a real URL — keep it
        else:
            result.append(part)
...
def _ensure_image_on_disk(image_data_url):
    """
    Saves a base64 data URL to UPLOAD_FOLDER if it isn't already a server path.
    Returns the '/get_image/<filename>' URL to store in memory, or None on failure.
    Used by ephemeral blob mode when an image arrived inline with no pre-saved file.
    """
    if not image_data_url or not image_data_url.startswith("data:"):
        return None
    try:
        header, b64 = image_data_url.split("base64,", 1)
        ...
        print(f"DEBUG (ephemeral blob): auto-saved inline image to {filepath}")
        return f"/get_image/{filename}"
    except Exception as _e:
        print(f"WARNING (ephemeral blob): could not save inline image: {_e}")
        return None
# =============================================================================
...
def chat_stream():
    ...
    # EPHEMERAL BLOB MODE: swap base64 blob for the server path so the image
    # stays renderable in UI on reload, but the fat blob never hits the JSON.
    if not VIDEO_SETTINGS.get("keep_image_blobs", False) and isinstance(user_payload.get("content"), list):
        _img_file = next((f for f in files_data if f and 'image' in f.get('type', '')), None)
        _sfn = _img_file.get("serverFilename") if _img_file else None
        _img_content = (_img_file or {}).get("content", "")
        _server_url = (f"/get_image/{os.path.basename(_sfn)}"
                       if _sfn else _ensure_image_on_disk(_img_content))
        _mem_to_save = copy.deepcopy(memory)
        _mem_to_save[-1]["content"] = _strip_blobs_for_memory(_mem_to_save[-1]["content"], server_url=_server_url)
        save_memory(_mem_to_save)
    else:
        save_memory(memory)
...
def analyze_image():
    ...
    # EPHEMERAL BLOB MODE: persist a copy where the base64 blob is swapped for
    # the slim server path so the image stays visible in the UI on reload,
    # but the fat blob is never written to the history JSON.
    # The live `memory` list stays untouched so the LLM gets the full blob THIS turn.
    if not VIDEO_SETTINGS.get("keep_image_blobs", False):
        _server_url = (f"/get_image/{os.path.basename(server_filename)}"
                       if server_filename else _ensure_image_on_disk(image_data_url))
        _mem_to_save = copy.deepcopy(memory)
        _mem_to_save[-1]["content"] = _strip_blobs_for_memory(_mem_to_save[-1]["content"], server_url=_server_url)
        save_memory(_mem_to_save)
    else:
...
def analyze_video():
    ...
    # --- Build multipart content — full blobs stored in memory ---
    # Mirrors analyze_image exactly: keep image_url parts intact so the KV
    # cache stays warm on the next turn. No stripping, no overrides needed.
    content_parts = [{"type": "text", "text": display_prompt}]
    for i, frame_data_url in enumerate(frames):
        ts = frame_timestamps[i] if i < len(frame_timestamps) else None
        if ts is not None:
            mm, ss = divmod(int(ts), 60)
            content_parts.append({"type": "text", "text": f"[Frame @ {mm}:{ss:02d}]"})
        content_parts.append({
            "type": "image_url",
            "image_url": {"url": frame_data_url}
        })

    # --- Save full multipart message to memory (blobs included) ---
    user_message_payload = {
        "role": "user",
    ...
    memory.append(user_message_payload)
    save_memory(memory)
    print(f"DEBUG: /analyze_video: Saved user message ({len(frames)} frames) to memory.")
...
def upload_video():
    ...
    for i, frame_data_url in enumerate(frames):
        ...
        if ts is not None:
            mm, ss = divmod(int(ts), 60)
            content_parts.append({"type": "text", "text": f"[Frame @ {mm}:{ss:02d}]"})
        content_parts.append({"type": "image_url", "image_url": {"url": frame_data_url}})
    ...
    # EPHEMERAL BLOB MODE: save each frame blob to FRAMES_DIR and swap the
    # base64 URLs for slim server paths in the persisted copy — frames stay
    # visible in the UI on reload, but the fat blobs never sit in the JSON.
    if not VIDEO_SETTINGS.get("keep_image_blobs", False):
        _mem_to_save = copy.deepcopy(memory)
        _saved_frame_paths = []
        for _part in _mem_to_save[-1].get("content", []):
            if _part.get("type") == "image_url":
                _url = _part.get("image_url", {}).get("url", "")
                if _url.startswith("data:"):
                    _fpath = _ensure_image_on_disk(_url)
                    _saved_frame_paths.append(_fpath)
                    if _fpath:
                        _part["image_url"]["url"] = _fpath
        save_memory(_mem_to_save)
        if _saved_frame_paths:
            print(f"DEBUG: /upload_video: Swapped {len(_saved_frame_paths)} frame blob(s) → server paths.")
    else:
        save_memory(memory)
    print(f"DEBUG: /upload_video: Saved user message ({len(frames)} frames) to memory.")
...
def set_video_settings():
    ...
    ALLOWED_KEYS = {"frame_divisor", "frame_floor", "frame_cap",
                    "jpeg_quality", "resolution", "keep_image_blobs"}
    unknown = set(data.keys()) - ALLOWED_KEYS
    if unknown:
        return jsonify({
    ...
    try:
        ...
        if "resolution" in data:
            ...
            data["resolution"] = val
        if "keep_image_blobs" in data:
            data["keep_image_blobs"] = bool(data["keep_image_blobs"])
