...
def _stamp_global_flags_multi(session_id_to_msg_ids: dict):
    ...
    for sid, msg_ids in session_id_to_msg_ids.items():
        if msg_ids:
            _stamp_msg_flags(sid, msg_ids, "indexed_global")
...
def _always_index_single_message(msg, session_id):
    ...
    if msg.get("indexed_local"):
        print(f"DEBUG (Always Index): Skipped [{msg.get('msg_id','no-id')}] — indexed_local already set.")
        return
    ...
    if RESONANCE_SETTINGS.get("chunking_enabled", False):
        ...
        _mid = msg.get("msg_id")
        if _mid:
            _stamp_msg_flags(session_id, {_mid}, "indexed_local")
...
def check_and_update_faiss_index(memory, active_memory, session_id):
    ...
    try:
        ...
        # Fallback: content-set for any pre-ID messages that slipped through backfill.
        ...
        new_unindexed = len([
            ...
            if not msg.get("indexed_local") and msg.get("content") not in indexed_message_contents
        ])
            ...
            if messages_to_flush:
                ...
                if RESONANCE_SETTINGS.get("chunking_enabled", False):
                    ...
                    if all_chunk_entries:
                        ...
                        if _flushed_ids:
                            _stamp_msg_flags(session_id, _flushed_ids, "indexed_local")
            ...
            if RESONANCE_SETTINGS.get("chunking_enabled", False):
                ...
                if all_chunk_entries:
                    ...
                    with _faiss_write_lock:
                        faiss.write_index(permanent_index, index_filepath)
                    save_faiss_metadata(metadata_filepath, indexed_messages)
                    # Stamp indexed_local on source messages
                    _quota_ids = {m.get("msg_id") for m in new_ghosted_messages if m.get("msg_id")}
                    if _quota_ids:
                        _stamp_msg_flags(session_id, _quota_ids, "indexed_local")
...
def delete_message():
    ...
    try:
        ...
        # Clear index flags on deleted messages so FAISS doesn't count orphaned vectors
        deleted_ids = {memory[i].get("msg_id") for i in indices_to_delete if i < len(memory) and memory[i].get("msg_id")}
        # Note: memory is already modified above so we rebuild from the saved file — flags already gone
...
def _wipe_all_faiss_indexes(reason=""):
    ...
    for session_folder in os.listdir(SESSION_DIR):
        ...
        if os.path.isdir(session_path):
            ...
            # Clear index flags so messages don't look pre-indexed after wipe
            _clear_msg_flags(session_folder, "indexed_local")
            _clear_msg_flags(session_folder, "indexed_global")
        ...
        try:
            if scope in ("local", "all"):
                ...
                try:
                    ...
                    memory = []
                active_memory = ghost_memory_if_needed(memory)
                # Clear indexed_local flags so every message looks fresh to the indexer
                _clear_msg_flags(session_id, "indexed_local")
            ...
            if scope in ("global", "all"):
                if RESONANCE_SETTINGS.get("global_memory_enabled", False):
                    ...
                    # Clear indexed_global flags across all sessions the global indexer covers
                    ...
                    try:
                        ...
                        for _sid in _all_sessions:
                            _clear_msg_flags(_sid, "indexed_global")
                    except Exception as _ce:
