...
# --- OPENCV (server-side video frame extraction) ---
try:
    import cv2
    _CV2_AVAILABLE = True
except ImportError:
    _CV2_AVAILABLE = False
    print("WARNING: opencv-python-headless not installed. /upload_video will be unavailable.")
...
def _backfill_msg_ids(memory: list) -> bool:
    ...
    dirty = False
    for msg in memory:
        if msg.get("role") in ("user", "assistant", "files", "system", "frame") and "msg_id" not in msg:
...
def _set_rebuild_running(val):
    """Thread-safe setter for _rebuild_running. Always use this instead of direct assignment
    in background threads — except _do_rebuild which manages the flag itself."""
    global _rebuild_running
    with _rebuild_status_lock:
        _rebuild_running = val
# -------------------------------------
...
# --- SYSTEM CONTENT HASH CACHE ---
# Keyed by (session_id, source_label) → hash of content last indexed.
# Prevents _index_system_content_immediately from re-embedding 700+ vectors
# every turn just because the persona is re-injected unchanged.
_system_content_hash_cache: dict = {}
...
def initialize_embedding_model(model_choice="nomic", ctx_length=None):
    ...
    if EMBEDDING_MODEL is not None:
        ...
        EMBEDDING_MODEL = None
        gc.collect()
    # Clear persona/system content hash cache so they re-index under new model dims
    _system_content_hash_cache.clear()
    print("DEBUG: Cleared system content hash cache (model switch).")
    # --------------------------------------------------
...
VIDEO_SETTINGS = {} # Will be loaded at startup
FRAMES_DIR = os.path.join("static", "video_frames")  # server-side extracted frames
...
def load_token_settings():
    ...
    defaults = {
        ...
        # --- BATCH EVICTION ---
        # Instead of drip-evicting one message per turn (seamless mode), batch eviction
        # holds the window until the threshold hits, then prunes a chunk all at once.
        # Think of it as a self-pruning buffer flush vs. a rolling eviction.
        # batch_eviction_size: how many oldest non-pinned messages to drop per flush.
        #   0 = auto (half of max_chat_messages). Frontend exposes this as an input box.
        # batch_eviction_enabled: master toggle. Frontend exposes this as a toggle button.
        "batch_eviction_enabled": False,
        "batch_eviction_size": 0
    }
...
RESONANCE_SETTINGS_DEFAULTS = {
    ...
    "chunk_code_aware": False,               # Code-aware splitter: splits at function/class/brace boundaries instead of sentences
...
def _split_code_blocks(text, filename_hint=""):
    """
    Code-aware text splitter. Detects language from filename_hint and splits
    at natural semantic boundaries (function/class definitions, top-level blocks)
    instead of paragraph/sentence breaks.

    Supported languages:
      - Python  (.py)          → splits at top-level `def ` / `class ` (col 0)
      - JS/TS   (.js .ts .jsx .tsx .mjs .cjs)  → splits by brace depth returning to 0
      - HTML    (.html .htm)   → splits at top-level tags (<div, <section, <script, etc.)
      - CSS     (.css .scss .less) → splits by brace depth returning to 0
      - Generic / unrecognised → returns [text] so caller falls back to prose splitting

    Each returned block is a complete semantic unit (full function, full class, etc.)
    so the embedding vector actually *means* something. Preserves all whitespace and
    ...
    if ext == ".py":
        ...
        for line in text.splitlines(keepends=True):
            is_top_def = line.startswith("def ") or line.startswith("class ")
            if is_top_def and current_lines:
    ...
    # ── JS / TS / CSS / SCSS ──────────────────────────────────────────────────
    if ext in (".js", ".ts", ".jsx", ".tsx", ".mjs", ".cjs", ".css", ".scss", ".less"):
        blocks = []
        current_lines = []
        depth = 0
        for line in text.splitlines(keepends=True):
            ...
            # Flush when we return to top-level scope and have content
            if depth <= 0 and any(c.strip() for c in current_lines):
                depth = 0  # guard against negative depth from comments/strings
                block = "".join(current_lines).strip()
                if block:
                    blocks.append(block)
                current_lines = []
        # Flush any trailing lines (e.g. top-level const/import lines)
        if current_lines:
            block = "".join(current_lines).strip()
            if block:
                blocks.append(block)
    ...
    # ── HTML / HTM ────────────────────────────────────────────────────────────
    if ext in (".html", ".htm"):
        # Split at top-level block tags — each <section>, <div>, <script>,
        # <style>, <head>, <body>, <article>, <main>, <header>, <footer>
        # opening tag starts a new block.
        _TOP_TAG_RE = re.compile(
            r'(?=<(div|section|article|main|header|footer|nav|aside|script|style|head|body)\b)',
            re.IGNORECASE
        )
        parts = _TOP_TAG_RE.split(text)
        # split() with a capturing group interleaves tag names — filter them out
        blocks = []
        i = 0
        while i < len(parts):
            piece = parts[i].strip()
            # skip the captured tag name tokens (they're short, single words)
            if piece and not re.fullmatch(r'\w+', piece):
                blocks.append(piece)
            i += 1
        return blocks if blocks else [text]

    # ── Unrecognised extension → signal caller to use prose splitting ─────────
    return [text]


def chunk_text(text, token_limit=None, overlap_tokens=None, return_overlap_info=False, filename_hint=""):
    """
    Splits text into chunks respecting natural boundaries.
    Strategy (prose): paragraph → sentence → word fallback.
    Strategy (code):  language-aware block split via _split_code_blocks() when
                      chunk_code_aware is ON and a recognised file extension is given.
    ...
    # --- 1. Split into segments (code-aware path OR prose path) ---
    #
    # Code-aware path: enabled when chunk_code_aware is ON and a filename_hint
    # with a recognised extension is provided. _split_code_blocks() returns [text]
    # unchanged for unrecognised extensions, so this safely falls through to prose.
    _code_aware = (
        RESONANCE_SETTINGS.get("chunk_code_aware", False)
        and bool(filename_hint)
    )
    if _code_aware:
        _code_segments = _split_code_blocks(text, filename_hint)
        # If the splitter returned something useful (more than one block, or a
        # single block that differs from the raw input), use those as segments.
        _splitter_fired = len(_code_segments) > 1 or (
            len(_code_segments) == 1 and _code_segments[0].strip() != text.strip()
        )
    else:
        _splitter_fired = False

    if _code_aware and _splitter_fired:
        # Code blocks are already semantic units — use them directly as segments.
        # If any individual block still exceeds token_limit, fall back to sentence
        # splitting for that block only (giant functions do exist).
        segments = []
        for block in _code_segments:
            if not block.strip():
                continue
            if count_tokens(block) <= token_limit:
                segments.append(block)
            else:
                # Oversized block: sentence-split it as a last resort
                ...
                if cur:
                    segments.append(cur)
        print(f"DEBUG (ChunkDLC code-aware): '{filename_hint}' → "
              f"{len(_code_segments)} block(s) → {len(segments)} segment(s).")
    else:
        # ── Original prose path ──────────────────────────────────────────────
        # --- 2a. Split into paragraphs first (double newline = paragraph boundary) ---
        raw_paragraphs = re.split(r'\n{2,}', text)
        paragraphs = [p.strip() for p in raw_paragraphs if p.strip()]

        # --- 2b. If any paragraph itself exceeds the limit, split by sentence ---
        segments = []
...
def _index_system_content_immediately(content, role, source_label, session_id, pinned=False, msg_id=None, filename_hint=""):
    """
    ...
    - filename_hint: passed through to chunk_text() to enable code-aware splitting
      when chunk_code_aware is ON and the file has a recognised extension.
    """
    ...
    # --- PERSONA DEDUP: skip if content hasn't changed since last index ---
    # Without this, every turn re-embeds ALL 700+ vectors just to update the
    # persona entry — a full hidden rebuild disguised as system content indexing.
    _cache_key = (session_id, source_label)
    _content_hash = hash(content)
    if _system_content_hash_cache.get(_cache_key) == _content_hash:
        print(f"DEBUG (ChunkDLC): '{source_label}' unchanged — skipping re-index (hash match).")
        return
    _system_content_hash_cache[_cache_key] = _content_hash
    print(f"DEBUG (ChunkDLC): '{source_label}' changed or new — re-indexing.")
    ...
    # --- Chunk the content ---
    chunks = chunk_text(content, return_overlap_info=True, filename_hint=filename_hint)
    total  = len(chunks)
...
def load_video_settings():
    """Loads video analysis settings from JSON or sets safe defaults."""
    global VIDEO_SETTINGS
    defaults = {
        "frame_divisor":     2,     # 1 frame per N seconds of video          (safe/conservative: 3)
        "frame_floor":       8,     # minimum frames regardless of clip length (safe/conservative: 4)
        "frame_cap":         200,   # server-side hard cap                     (safe/conservative: 60)
        "jpeg_quality":      0.92,  # JPEG compression quality (0.1–1.0)       (safe/conservative: 0.82)
        "resolution":        720,   # MAX_DIM longest edge cap in pixels        (safe/conservative: 512)
        "keep_image_blobs":  True,  # Always vanilla — blobs kept in history
    }
    if os.path.exists(VIDEO_CONFIG_FILE):
        try:
            ...
            print("DEBUG: Loaded video settings.")
        except (json.JSONDecodeError, OSError) as e:
            print(f"WARNING: {VIDEO_CONFIG_FILE} unreadable ({e}). Using defaults.")
            VIDEO_SETTINGS = dict(defaults)
    else:
        print(f"INFO: {VIDEO_CONFIG_FILE} not found. Creating with defaults.")
        VIDEO_SETTINGS = dict(defaults)
    save_video_settings()


def save_video_settings():
    """Saves the current video settings to JSON."""
    try:
        with open(VIDEO_CONFIG_FILE, "w", encoding="utf-8") as f:
            json.dump(VIDEO_SETTINGS, f, indent=4)
        print("DEBUG: Saved video settings.")
    except Exception as e:
        print(f"ERROR: Failed to save video settings: {e}")


# =============================================================================
# (Ephemeral blob helpers removed — vanilla mode only, blobs always kept in history)
...
def get_system_prompt():
    ...
    # Resolve {{user}} live from the active loadout so it never goes stale.
    # Card body fields are stored with {{user}} preserved as a literal at import time.
    _live_user = get_current_user_loadout_name() or 'User'
    full_prompt_content = re.sub(r'\{\{user\}\}', _live_user, full_prompt_content, flags=re.IGNORECASE)
...
def duckduckgo_search(query: str, log_file: str = "search_log.jsonl") -> list:
    ...
    try:
        ...
        for r in results:
            ...
            # --- Scraping Logic ---
            # universal_scraping=True  → scrape any domain (experimental, no allowlist)
            # universal_scraping=False → fall back to the scrapeable_domains allowlist (default)
            universal_scraping = SEARCH_SETTINGS.get("universal_scraping", False)
            should_scrape = False
            if link:
                if universal_scraping:
                    should_scrape = True
                    print(f"[🌐] Universal scraping enabled — scraping: {link}")
                elif scrapeable_domains:
            ...
            if should_scrape:
                try:
                    ...
                    # Re-inject the date tag into scraped content so the AI still sees it
                    if extracted_date:
...
def _wait_for_global_rebuild(timeout=30):
    """
    Block until any in-progress global index rebuild finishes, then release immediately.
    Used by recall paths so they don't query an empty index mid-rebuild.
    Returns True if the lock was acquired (rebuild done or nothing was running),
    ...
    if acquired:
        ...
        return True
    print("DEBUG (Global Index): Timed out waiting for rebuild to complete.")
    return False
...
def _rebuild_global_index_inner(force=False):
    ...
    try:
        ...
        print(f"\n{'='*60}")
        print(f"[GLOBAL-INDEX] mode={'FULL REBUILD (force=True)' if force else 'INCREMENTAL (force=False)'}")
        print(f"               candidate messages across {len(sessions_to_index)} session(s): {len(all_messages)}")
        print(f"{'='*60}")
        ...
        if not force and os.path.exists(global_index_path) and os.path.exists(global_meta_path):
            try:
                ...
                if global_index.d != EMBEDDING_DIM:
                    ...
                    # FIX (chunk repeat): In precision mode each chunk gets a unique
                    # msg_id, so the source message's own msg_id is never in the
                    # chunk-only set. This caused the same N source messages to expand
                    # into chunks on EVERY incremental pass. Adding parent_msg_ids
                    # ensures source messages are recognised as already-indexed.
                    already_indexed         = {m.get("msg_id") for m in indexed_meta if m.get("msg_id")}
                    already_indexed.update(m.get("parent_msg_id") for m in indexed_meta if m.get("parent_msg_id"))
        ...
        if not new_messages and global_index is not None:
            print(f"[GLOBAL-INDEX] ✅ NOTHING TO DO — all {len(all_messages)} message(s) already indexed. {global_index.ntotal} vectors unchanged.\n")
            return

        if new_messages:
            print(f"[GLOBAL-INDEX] delta={len(new_messages)} new | skipped={len(all_messages)-len(new_messages)} already indexed | encoding now...")
            print(f"               → {'INCREMENTAL .add() — NOT a full rebuild' if not force else 'FULL REBUILD — re-encoding everything from scratch'}\n")
...
def ghost_memory_if_needed(memory):
    ...
    for idx, msg in enumerate(memory):
        ...
        # BATCH EVICTION: skip messages flagged as evicted — they stay on disk for FAISS
        # but are invisible to the LLM. Window sees only the unflagged tail.
        if msg.get("_batch_evicted"):
            continue
        
        # HOLLOWS: Search results, recalled memories, RAG injections, and video frames.
        # These are WEIGHTLESS - present in context but don't count toward window!
        # Frame messages (role="frame") are disposable image entities that slide out
        # naturally with the conversation — never counted against the turn limit.
        if msg.get("_search_result") or msg.get("_recall_result") or msg.get("_raw_rag") or msg.get("role") == "frame":
            is_hollow = True
    ...
    # Seamless per-turn eviction — oldest messages drop out of the LLM view as new ones arrive.
    # Batch eviction is handled physically in chat_stream (post Final Save), not here.
    # ghost_memory_if_needed is a READ-ONLY view filter — it never mutates chat_memory.json.
...
def inject_silent_intro_route():
    ...
    formatted_intro = f"MEMORY: {summarized_intro}"
    ...
    try:
        ...
        if 0 <= index_to_edit < len(memory):
            ...
            if (message_to_edit.get("role") == "system" and
                ...
                # FIX BUG 20: Re-apply the "MEMORY: " prefix that inject_silent_intro
                ...
                memory[index_to_edit]["content"] = f"MEMORY: {new_content.strip()}"
                save_memory(memory)
...
def _always_index_single_message(msg, session_id):
    ...
    # FIX: flatten list content BEFORE the preview line so .replace() never
    # crashes on a list (happens when user message contains image_url parts)
    if isinstance(content, list):
        content = " ".join(p.get("text", "") for p in content if p.get("type") == "text")

    # ── INDEXING DECISION BANNER ──────────────────────────────────────────────
    _mid_preview = msg.get("msg_id", "no-id")
    _content_preview = (content or "")[:60].replace("\n", " ")
    print(f"\n{'='*60}")
    print(f"[ALWAYS-INDEX] role={role} | id={_mid_preview}")
    print(f"               content_preview: {_content_preview!r}")
    print(f"{'='*60}")
    ...
    # If global is ON  → incrementally add this single message to the global index.
    #                    Same load → dedup → embed 1 → .add() → atomic write pattern
    #                    as the local path. No full rebuild, no debounce, instant.
    # If global is OFF → fall through to local write path below.
    if RESONANCE_SETTINGS.get("global_memory_enabled", False):
        global_index_path = get_global_faiss_filepath()
        global_meta_path  = get_global_faiss_metadata_filepath()

        # --- Dedup: global flag (primary) ---
        if msg.get(_get_global_flag_key()):
            print(f"DEBUG (Always Index/Global): Skipped [{msg.get('msg_id','no-id')}] — {_get_global_flag_key()} already set.")
            return

        # --- Load existing global index ---
        g_index = None
        g_messages, g_model = [], None
        if os.path.exists(global_index_path) and os.path.exists(global_meta_path):
            try:
                _t = _safe_faiss_read(global_index_path)
                _gm, _gmod = load_faiss_metadata(global_meta_path)
                cur_model = RESONANCE_SETTINGS.get("embedding_model", "nomic")
                if _t.d != EMBEDDING_DIM:
                    print(f"DEBUG (Always Index/Global): Dim mismatch ({_t.d} vs {EMBEDDING_DIM}). Starting fresh.")
                elif _gmod is not None and _gmod != cur_model:
                    print(f"DEBUG (Always Index/Global): Model mismatch ('{_gmod}' vs '{cur_model}'). Starting fresh.")
                else:
                    g_index, g_messages, g_model = _t, _gm, _gmod
            except Exception as _ge:
                print(f"DEBUG (Always Index/Global): Could not load global index ({_ge}). Starting fresh.")

        if g_index is None:
            g_index = create_faiss_index(EMBEDDING_DIM)
            g_messages = []

        # --- Dedup: content fallback ---
        g_indexed_contents = {m.get("content", "") for m in g_messages}
        if content in g_indexed_contents:
            print(f"DEBUG (Always Index/Global): Skipped duplicate — already in global index (content fallback).")
            return

        # --- CHUNKING DLC for global ---
        if RESONANCE_SETTINGS.get("chunking_enabled", False):
            chunk_pairs   = chunk_text(content, return_overlap_info=True)
            new_chunk_pairs = [(c, op) for c, op in chunk_pairs if c not in g_indexed_contents]
            new_chunks    = [c for c, _ in new_chunk_pairs]
            if not new_chunks:
                print("DEBUG (Always Index/Global + ChunkDLC): All chunks already in global index.")
                return
            total = len(chunk_pairs)
            ...
            try:
                ...
                print(f"DEBUG (Always Index/Global + ChunkDLC): Embedding failed ({_emb_exc}). Skipping.")
                return
            g_index.add(vecs_np)
            _precision_mode = RESONANCE_SETTINGS.get("chunk_retrieval_mode", "precision") == "precision"
            for idx, (chunk, overlap_prefix) in enumerate(new_chunk_pairs):
                ...
                _chunk_mid  = _new_msg_id() if _precision_mode else _parent_mid
                g_messages.append({
                    "role": role, "name": msg.get("name", ""),
                    "content": chunk, "chunk_source": source_label,
                    "chunk_index": idx, "chunk_total": total,
                    "overlap_prefix": overlap_prefix, "pinned": False,
                    "timestamp": msg.get("timestamp", ""),
                    "msg_id": _chunk_mid, "parent_msg_id": _parent_mid,
                    "session_source": session_id, "always_indexed": True
                })
            _atomic_index_write(g_index, global_index_path, global_meta_path, g_messages)
            _mid = msg.get("msg_id")
            if _mid:
                _stamp_msg_flags(session_id, {_mid}, _get_global_flag_key())
                # FIX (BACKLOG): Also stamp local flag so the recovery flush-check
                # finds nothing to do. The global path returns early without writing
                # to the local index, leaving local flag unset, which caused
                # BACKLOG DETECTED on every turn even though nothing was wrong.
                _stamp_msg_flags(session_id, {_mid}, _get_local_flag_key())
            print(f"DEBUG (Always Index/Global + ChunkDLC): [{role}] → {len(new_chunks)} chunk(s) added. "
                  f"Total global vectors: {g_index.ntotal}")
            return

        # --- Single vector encode + add ---
        try:
            raw = embed_documents([content], show_progress_bar=False)
            vec = np.array(raw, dtype="float32")
            vec = prepare_vectors(vec)
        except Exception as _emb_exc:
            print(f"DEBUG (Always Index/Global): Embedding failed ({_emb_exc}). Skipping.")
            return

        g_index.add(vec)
        g_messages.append({
            "role": role, "name": msg.get("name", ""),
            "content": content, "timestamp": msg.get("timestamp", ""),
        ...
        if _mid:
            _stamp_msg_flags(session_id, {_mid}, _get_global_flag_key())
            # FIX (BACKLOG): Also stamp local flag — same reason as chunked path above.
            _stamp_msg_flags(session_id, {_mid}, _get_local_flag_key())
        print(f"DEBUG (Always Index/Global): Appended [{role}] ({len(content)} chars) → "
              f"{g_index.ntotal} total vectors in global index.")
        return
    # --- END GLOBAL ROUTING ---
    ...
    if existing_index is None:
        existing_index = create_faiss_index(EMBEDDING_DIM)
        indexed_messages = []
        print(f"[ALWAYS-INDEX] ⚡ FRESH local index — no existing index found, building from scratch.")
    else:
        print(f"[ALWAYS-INDEX] ✅ INCREMENTAL — existing local index has {existing_index.ntotal} vector(s). Will append only.")
    ...
    if content in indexed_contents:
        ...
        # FIX (IDLE stamp gap): content is already in the index but this specific message's
        # flag was never stamped — happens every repeated IDLE turn after the first one,
        # since "⏩" content deduplicates but each turn gets a fresh msg_id with no flag.
        # Without this stamp, FLUSH-CHECK fires a BACKLOG warning on the very next turn
        # even though no actual re-indexing is needed.
        _mid = msg.get("msg_id")
        if _mid:
            _stamp_msg_flags(session_id, {_mid}, _get_local_flag_key())
        return
    ...
    if RESONANCE_SETTINGS.get("chunking_enabled", False):
        ...
        if not new_chunks:
            ...
            # FIX (IDLE stamp gap): same as content-dedup path above — all chunks exist
            # in the index but this message's flag was never stamped. Stamp it now so
            # FLUSH-CHECK doesn't fire a false BACKLOG on the next turn.
            _mid = msg.get("msg_id")
            if _mid:
                _stamp_msg_flags(session_id, {_mid}, _get_local_flag_key())
            return
    ...
    if _mid:
        _stamp_msg_flags(session_id, {_mid}, _get_local_flag_key())
    print(f"[ALWAYS-INDEX] ✅ DONE — appended 1 vector [{role}] ({len(content)} chars). "
          f"Local index now has {existing_index.ntotal} total vector(s).")
    print(f"               → This was INCREMENTAL (1 new vector added, nothing re-encoded).\n")


# --- NEW: AUTOMATIC FAISS INDEXING FUNCTION ---
def check_and_update_faiss_index(memory, active_memory, session_id, skip_global=False):
    """
    Automatically checks if the Faiss index needs to be updated and does so.
    This runs independently of memory recall triggers.

    skip_global: When True, suppresses the internal rebuild_global_index() calls.
                 Use this when the caller (e.g. manual "all" scope rebuild) will
                 handle the global rebuild explicitly afterward, to avoid running
                 global twice.
    """
    use_permanent_index = RESONANCE_SETTINGS.get("faiss_permanent_indexing", False)
    if not use_permanent_index:
    ...
    try:
        ...
        # --- Accurate per-tier breakdown for the debug line ---
        # active_memory is the full output of ghost_memory_if_needed — it contains three tiers:
        #   Tier 1: preserved system headers (persona, intros, facts)
        #   Tier 2: hollows (search results, recalled memories, RAG) — weightless, don't count toward window
        #   Tier 3: real conversation (user / assistant / files) — the ONLY tier the window limit applies to
        # Lumping all three into one Active(visible)=N/max_messages number was misleading:
        # 5 recall blocks + 4 user/assistant messages would show Active(visible)=9/10 even though
        # the window is nowhere near full, making the sliding window look broken.
        _active = active_memory if active_memory else []
        _conv_active   = sum(1 for m in _active if m.get("role") in ("user", "assistant", "files"))
        _hollow_active = sum(1 for m in _active if m.get("_search_result") or m.get("_recall_result") or m.get("_raw_rag"))
        _sys_active    = sum(1 for m in _active if m.get("role") == "system" and not m.get("_search_result") and not m.get("_recall_result") and not m.get("_raw_rag"))
        print(
            f"DEBUG (Sliding Window): "
            f"Conv={_conv_active}/{max_messages} | "
            f"Ghosted(conv)={ghosted_count} | "
            f"Hollows(active)={_hollow_active} | "
            f"System(preserved)={_sys_active} | "
            f"Total in memory={total_messages} | "
        ...
        if _always_index_on:
            ...
            if RESONANCE_SETTINGS.get("global_memory_enabled", False) and not skip_global:
                rebuild_global_index()
                print("DEBUG (Faiss): AUTO-INDEX: Always Index ON + Global ON — synced global index. Continuing to local flush.")
            elif RESONANCE_SETTINGS.get("global_memory_enabled", False) and skip_global:
                print("DEBUG (Faiss): AUTO-INDEX: Always Index ON + Global ON — skipping internal global sync (caller will rebuild global explicitly).")
            # --- END GLOBAL ROUTING ---
            ...
            if messages_to_flush:
                print(f"\n{'='*60}")
                print(f"[FLUSH-CHECK] ⚠️  BACKLOG DETECTED — {len(messages_to_flush)} unindexed message(s) need encoding.")
                print(f"              This means dedup flags were missed on a previous turn!")
                for _m in messages_to_flush:
                    _r = _m.get('role','?')
                    _id = _m.get('msg_id','no-id')
                    _prev = (_m.get('content','')[:50] or '').replace('\n',' ')
                    print(f"              → [{_r}] {_id}: {_prev!r}")
                print(f"{'='*60}")
                start_time = time.time()
                ...
                print(f"\n[FLUSH-CHECK] ✅ CLEAN — no backlog. All {len(all_conversation_messages)} conversation message(s) already indexed. Real-time mode working correctly.\n")
            return
...
def get_relevant_memories(user_message, memory, active_memory=None, session_id=None, force_search=False, raw_mode=True):
    ...
    try:
        ...
        # FIX (chunk pre-filter blind spot): also collect msg_ids of active messages.
        # When chunking is ON, FAISS stores chunk substrings — chunk_content never equals
        # a full message string in active_contents_set, so the content check silently misses
        # chunks whose parent is still live. The msg_id check catches these cases.
        active_contents_set = set()
        active_msg_ids_set = set()
        if active_memory:
        ...
        if use_global_memory:
            ...
            def _do_global_search():
                """
                Load and search the global index. Returns True if a search was
                performed (even with 0 results), False if the index is unusable.
                Appends hits directly into all_results.
                """
                if not (os.path.exists(global_index_path) and os.path.exists(global_meta_path)):
                    return False
                try:
                    g_index = _safe_faiss_read(global_index_path)
                    g_msgs, g_model = load_faiss_metadata(global_meta_path)
                    cur_model = RESONANCE_SETTINGS.get("embedding_model", "nomic")
                    if g_index.d != EMBEDDING_DIM or g_index.ntotal == 0 or \
                       (g_model is not None and g_model != cur_model):
                        return False  # stale — caller will rebuild
                    search_k = min(retrieval_k, g_index.ntotal)
                    distances, indices = g_index.search(query_vector_np, search_k)
                    for dist, i in zip(distances[0], indices[0]):
                        if i != -1:
                            msg = g_msgs[i]
                            _g_active = (
                                msg.get("content", "").strip() in active_contents_set
                                or msg.get("msg_id") in active_msg_ids_set
                                or msg.get("parent_msg_id") in active_msg_ids_set
                            )
                            if _g_active:
                                print(f"DEBUG (Global Index): Skipping result already in active window.")
                                continue
                            all_results.append((dist, msg))
                    # Expose the messages list so paired/reform paths can use it
                    global_messages.extend(g_msgs)
                    print(f"DEBUG (Global Index): Search returned {len(all_results)} raw results (after active window filter).")
                    return True
                except Exception as e:
                    print(f"ERROR (Global Index): Search failed: {e}.")
                    return False

            if not _do_global_search():
                # Index missing or stale — ensure a clean rebuild is complete before retrying.
                _reason = "No global index found yet" if not os.path.exists(global_index_path) \
                          else "Index stale (dim or model mismatch)"
                print(f"DEBUG (Global Index): {_reason}. Triggering rebuild + retry...")

                # GUARD: If a background rebuild is already running, WAIT for it to finish
                # instead of falling through immediately to the temp fallback on an empty index.
                # If nothing is running, trigger a rebuild ourselves. Either way, only retry
                # _do_global_search() once we know the index is actually ready.
                if not _rebuild_lock.acquire(blocking=False):
                    print("DEBUG (Global Index): Rebuild already in progress — waiting for completion before retry...")
                    _wait_for_global_rebuild(timeout=30)
                else:
                    # Lock acquired means nothing was running — release and do it ourselves.
                    # Use force=True when the file is missing so the debounce doesn't skip us —
                    # there's nothing to serve from, cooldown is irrelevant.
                    _rebuild_lock.release()
                    rebuild_global_index(force=not os.path.exists(global_index_path))

                if not _do_global_search():
                    print("DEBUG (Global Index): Post-rebuild search still empty — continuing with temp fallback.")
        else:
                    ...
                    if perm_index.d == EMBEDDING_DIM and perm_index.ntotal > 0 and (index_model is None or index_model == current_model):
                        ...
                        for dist, i in zip(distances[0], indices[0]):
                            if i != -1:
                                ...
                                # Check BOTH content string (non-chunked path) AND msg_id
                                # (chunked path — chunk substring ≠ full message string,
                                # so content check alone silently misses active chunks).
                                _is_active = (
                                    msg_data.get("content", "").strip() in active_contents_set
                                    or msg_data.get("msg_id") in active_msg_ids_set
                                    or msg_data.get("parent_msg_id") in active_msg_ids_set
                                )
                                if _is_active:
        ...
        if RESONANCE_SETTINGS.get("chunking_enabled", False) and \
           RESONANCE_SETTINGS.get("chunk_retrieval_mode", "precision") == "long_context":
            ...
            if RESONANCE_SETTINGS.get("paired_memory_enabled", False) and reformed_messages:
                ...
                def _add_pair(msg):
                    c = msg.get("content", "")
                    # FIX (reform content-mismatch): also block by msg_id.
                    # _reform_by_mid joins chunks with "\n" — the reconstructed string
                    # can differ from the original content in active_set even though
                    # the parent message is still live. Check the ID as a fallback.
                    _mid = msg.get("msg_id")
                    if c in seen_pair_content:
                        return
                    if c.strip() in active_set:
                        return
                    if _mid and _mid in active_msg_ids_set:
                        return
                ...
                for r_msg in reformed_messages:
                    ...
                    if r_role == "user":
                        ...
                        # FIX (Paired+LongCtx): skip sibling chunks of the same parent message.
                        # mid_to_mem_idx stores only the FIRST chunk index for each parent_mid.
                        # Without the skip, chunk_1 of a multi-chunk user message has role="user"
                        # and triggers the "two users in a row" break before the real assistant
                        # reply is ever reached — partner is silently dropped every time.
                        ni = mem_idx + 1
                        while ni < len(_lc_pair_pool):
                            nb = _lc_pair_pool[ni]
                            nb_mid  = nb.get("msg_id")
                            nb_role = nb.get("role", "")
                            if nb_mid == r_mid:          # sibling chunk of this same message
                                ni += 1
                                continue
                        ...
                        # FIX (Paired+LongCtx): same sibling-skip as the forward walk above.
                        # Without it, chunk_N-1 of a multi-chunk assistant message (role="assistant")
                        # triggers the "two assistants in a row" break before the real user
                        # message before it is ever reached.
                        pi = mem_idx - 1
                        while pi >= 0:
                            nb = _lc_pair_pool[pi]
                            nb_mid  = nb.get("msg_id")
        ...
        if RESONANCE_SETTINGS.get("paired_memory_enabled", False) and not _long_context_handled_pairing and not _precision_lane:
            ...
            #
            # FIX: The fallback pool previously filtered isinstance(content, str),
            # which silently excluded multimodal user messages (content is a list).
            # _make_pair_pool extracts the text part of list-content messages so
            # image turns are visible to the pair walk.
            def _make_pair_pool(mem):
                pool = []
                for _pm in mem:
                    if _pm.get("role") not in ("user", "assistant"):
                        continue
                    _pc = _pm.get("content", "")
                    if isinstance(_pc, str) and _pc.strip():
                        pool.append(_pm)
                    elif isinstance(_pc, list):
                        _pt = " ".join(p.get("text","") for p in _pc if p.get("type")=="text").strip()
                        if _pt:
                            pool.append({**_pm, "content": _pt})
                return pool
            try:
                if use_global_memory:
                ...
                if not _pair_pool:
                    _pair_pool = _make_pair_pool(memory)
            except Exception:
                _pair_pool = _make_pair_pool(memory)
        ...
        for msg in final_matched_messages:
            ...
            if isinstance(_raw_content, list):
                # Multimodal message (image turn) — extract text parts only.
                # The blob is not recalled (meaningless as text) but the prompt is.
                msg_content = " ".join(
                    p.get("text", "") for p in _raw_content if p.get("type") == "text"
                )[:char_limit]
            else:
                msg_content = str(_raw_content)[:char_limit]
...
def _dedup_recall_block(new_reflection, memory, extra_active_contents=None, session_id=None):
    ...
    # FIX (reform content-mismatch): also track active msg_ids so the loop below
    # can block by ID when content-string comparison fails.  _reform_by_mid stitches
    # chunks with "\n".join() — the joined string may differ from the original message
    # content (e.g. original used spaces at chunk boundaries), so `content in active_contents`
    # silently misses reformed entries whose parent IS live in the active window.
    active_msg_ids = {
        msg.get("msg_id") for msg in active_window if msg.get("msg_id")
    }
    ...
    if session_id and RESONANCE_SETTINGS.get("chunking_enabled", False):
        try:
            ...
            # active_msg_ids is already seeded above from active_window — here we
            # extend it with any msg_ids embedded in active recall block labels.
    ...
    # Pre-compile pattern once for label msg_id extraction below.
    # Reuse the same pattern already used in the chunk expansion block above.
    import re as _re_bouncer
    _label_mid_pat = _re_bouncer.compile(r'\b(mid_[0-9a-f]{16})\b')
    ...
    for label, content in incoming_entries:
        ...
        # FIX (reform content-mismatch): extract msg_id from the label and check it
        # against active_msg_ids as a secondary guard.
        # When ChunkDLC reforms a message it stitches chunks with "\n".join() — the
        # joined string can differ from the original (e.g. original had spaces at
        # chunk boundaries, reform introduces newlines), so `content in active_contents`
        # silently passes the entry even though its parent IS still live in the window.
        # Label format: [Name | timestamp | mid_xxxxxxxxxxxxxxxx]:
        _lm = _label_mid_pat.search(label)
        _label_mid = _lm.group(1) if _lm else None
        _id_is_active = bool(_label_mid and _label_mid in active_msg_ids)

        if content in active_contents or _id_is_active:
            # Already visible to the LLM right now — safe to skip.
            # _id_is_active catches reformed messages where the content string
            # comparison fails due to chunk-stitching whitespace differences.
            skipped_active += 1
...
def chat_stream():
    ...
    file_data = data.get("file")       # legacy single-file (kept for compat)
    files_data = data.get("files", []) # new multi-file array

    # Normalise: merge legacy single file into the array
    ...
    for file_data in files_data:
        if not file_data or 'image' in file_data.get('type', ''):
            continue  # skip images — they route through /analyze_image
        filename = file_data.get("name")
        content  = file_data.get("content")
        if not (filename and content):
            continue

        # PDF base64 data URL -> extract text via pypdf
        if file_data.get('type') == 'application/pdf' and isinstance(content, str) and content.startswith('data:'):
            try:
                import base64 as _b64mod, io as _io
                _b64 = content.split(',', 1)[1]
                _pdf_bytes = _b64mod.b64decode(_b64)
                try:
                    from pypdf import PdfReader
                    _reader = PdfReader(_io.BytesIO(_pdf_bytes))
                    _pages_text = '\n'.join(
                        (_reader.pages[i].extract_text() or '') for i in range(min(len(_reader.pages), 50))
                    )
                    content = _pages_text if _pages_text.strip() else '[PDF: no extractable text]'
                except Exception as _pe:
                    content = '[PDF: text extraction failed]'
                    print(f"DEBUG: PDF extraction failed for '{filename}': {_pe}")
            except Exception as _de:
                content = '[PDF: could not decode]'
        ...
        elif RESONANCE_SETTINGS.get("always_index_messages", False) and \
             RESONANCE_SETTINGS.get("chunk_index_system_immediately", True):
            # Guard: if chunk_index_system_immediately is OFF, the user explicitly
            # does not want system/injection content indexed — respect that here too.
            # Without this guard the elif fires as a fallback and indexes the file
            # anyway, bypassing the setting entirely.
            try:
    ...
    # Idle-mode sends pass silent:true so the prompt is stored as a hidden
    # message — reloadChat() skips entries where silent is truthy, keeping
    # the idle prompt invisible on re-render and after page refreshes.
    if data.get("silent"):
        user_payload["silent"] = True
    
    llm_content_parts = []
    text_prompt = user_message
    image_files = [f for f in files_data if f and 'image' in f.get('type', '')]
    if not text_prompt and image_files:
        text_prompt = "Describe this image in detail."

    if text_prompt:
        llm_content_parts.append({"type": "text", "text": text_prompt})
    # Add ALL images — previously only the last one was added due to a loop-variable leak
    for _img in image_files:
        llm_content_parts.append({"type": "image_url", "image_url": {"url": _img.get("content")}})
    ...
    if llm_content_parts:
        ...
        # File-only send: no text prompt was typed. Still save a visible placeholder
        # so reloadChat() has a user bubble to render after streaming finishes.
        sent_names = [f.get("name", "file") for f in files_data if f and not 'image' in f.get('type', '')]
        if sent_names:
            label = ", ".join(sent_names)
            user_payload['content'] = f"📎 *[File attached: {label}]*"
        else:
            user_payload['content'] = "📎 *[File attached]*"
    ...
    # --- ALWAYS INDEX: User message is indexed AFTER assistant response ---
    # (moved below to keep sequence: intent → recall → LLM → index user+assistant together)
    # _always_index_single_message(user_payload, session_id)  ← was here, causes CPU spike before LLM
    ...
    # NOTE: check_and_update_faiss_index intentionally removed from here.
    # It used to run synchronously before RAG, competing with LLM generation.
    # Now it fires in a background thread at the END of each turn (post Final Save)
    # so indexing never blocks or steals CPU from RAG + streaming.
    # _always_index_single_message already keeps the index warm turn-by-turn,
    # so RAG below will always find a fresh index without a blocking flush here.
    ...
    def _agent_response_generator_inner(_last_acc, _is_saved):
        ...
        for msg in active_memory:
            if msg.get("_img_companion"):
                continue  # indexing-only entry — the real multimodal user msg is already in context
            msg_for_llm = {k: v for k, v in msg.items() if k not in _INTERNAL_MSG_KEYS}
            # Remap non-standard roles to "system" — OpenRouter (and strict
            # OpenAI-compatible backends) reject any role outside user/assistant/system.
            if msg_for_llm.get("role") in ("files", "frame", "memory"):
        ...
        while current_turn < max_agent_turns:
            ...
            # --- BATCH EVICTION: bottom-up slice + sphincter guard ---
            # N1 = newest message (tail), N60 = oldest visible (window floor).
            # Anything beyond the floor gets _batch_evicted — stays on disk for FAISS,
            # invisible to LLM. Once flagged, STAYS flagged (sphincter — no creep-back).
            if TOKEN_SETTINGS.get("batch_eviction_enabled", False):
                _max_msgs  = TOKEN_SETTINGS.get("max_chat_messages", 16)
                _batch_sz  = TOKEN_SETTINGS.get("batch_eviction_size", 0)
                # Only count real pinned headers — same definition as ghost_memory_if_needed
                # NOT evicted messages, NOT hollows — just system/silent that always stay
                _preserved = [
                    m for m in memory
                    if m.get("role") == "system"
                    and not m.get("_batch_evicted")
                    and not m.get("_search_result") and not m.get("_recall_result")
                    and not m.get("_raw_rag")
                ]
                _max_convo = max(2, _max_msgs - len(_preserved))
                _eff_batch = _batch_sz if _batch_sz > 0 else _max_convo // 2

                # Bottom-up: grab only ACTIVE (non-evicted) convo messages in chronological order
                _all_convo = [
                    m for m in memory
                    if m.get("role") in ("user", "assistant")
                    and not m.get("_batch_evicted")
                    and not m.get("_search_result") and not m.get("_recall_result")
                    and not m.get("_raw_rag") and m.get("role") != "frame"
                ]

                # The active window is always the tail slice — N1 at end, Nmax at start
                _active_window = _all_convo[-_max_convo:]
                _outside       = _all_convo[:-_max_convo]  # everything older than the window

                # SPHINCTER: re-flag anything outside the window that lost its flag somehow
                _dirty = False
                for m in _outside:
                    if not m.get("_batch_evicted"):
                        m["_batch_evicted"] = True
                        _dirty = True

                # FLUSH: only fire when the active window is exactly full — clean math every cycle
                if len(_active_window) == _max_convo:
                    _to_flag  = _active_window[:_eff_batch]
                    _flag_ids = {m["msg_id"] for m in _to_flag if m.get("msg_id")}
                    for m in memory:
                        if m.get("msg_id") in _flag_ids:
                            m["_batch_evicted"] = True
                    _dirty = True
                    print(f"[batch_eviction] Window full ({len(_active_window)}/{_max_convo}). "
                          f"Flushed oldest {len(_to_flag)}. "
                          f"Active window drops to ~{_max_convo - len(_to_flag)}. "
                          f"Ghosted total: {len(_outside) + len(_to_flag)}. FAISS unaffected.")

                if _dirty:
                    save_memory(memory)
            # --- END BATCH EVICTION ---

            # --- POST-TURN INDEXING: always_index + recovery flush run in background ---
            # Moved entirely out of the hot path. Previously this ran synchronously
            # before the next request's RAG, causing CPU contention with embedding +
            # LLM generation. Now it fires in a daemon thread immediately after
            # Final Save — the client already has the full response at this point.
            #
            # Order inside the thread:
            #   1. Index user message  (incremental, local + global)
            #   2. Index assistant message (incremental, local + global)
            #   3. Recovery flush-check — with always_index ON this should be a
            #      no-op (finds 0 backlog) but catches any edge-case missed stamps.
            if RESONANCE_SETTINGS.get("always_index_messages", False):
                _user_snap = user_payload
                _asst_snap = memory[-1]
                _sid_snap  = session_id
                def _post_turn_index_worker(_u=_user_snap, _a=_asst_snap, _sid=_sid_snap):
                    try:
                        if _u and _u.get("content"):
                            _always_index_single_message(_u, _sid)
                    except Exception as _e:
                        print(f"DEBUG (Post-Turn Index): user index failed: {_e}")
                    try:
                        _always_index_single_message(_a, _sid)
                    except Exception as _e:
                        print(f"DEBUG (Post-Turn Index): asst index failed: {_e}")
                    # Recovery flush-check: should find 0 backlog when always_index
                    # is working correctly. Catches crashes / missed stamps.
                    # skip_global=True: _always_index_single_message already handled
                    # global incrementally above — no need to call rebuild_global_index
                    # again here, which was causing the delta=12 repeat every turn.
                    try:
                        _mem = load_memory()
                        _active = ghost_memory_if_needed(_mem)
                        check_and_update_faiss_index(_mem, _active, _sid, skip_global=True)
                    except Exception as _e:
                        print(f"DEBUG (Post-Turn Index): flush-check failed: {_e}")
                threading.Thread(target=_post_turn_index_worker, daemon=True).start()
            break
...
def analyze_image():
    ...
    # Resolve display filename early — used both in the user message text and
    # the companion entry so the filename is baked into the conversation turn
    # exactly like uploaded files are, making it naturally recallable.
    _original_filename = data.get("original_filename") or data.get("display_filename")
    _img_filename      = _original_filename or server_filename or "inline_image"
    _prompt_with_file  = f"{prompt} [{_img_filename}]"

    # 2. Save the user's multimodal message to memory to maintain context
    user_message_payload = {
        "role": "user",
        "content": [
            {"type": "text", "text": _prompt_with_file},
    ...
    # --- IMAGE COMPANION ENTRY ---
    # Mirrors the files upload pattern (lines ~5727-5757).
    # The multimodal user message above has content as a list [text, image_url]
    # which all indexers skip (not a plain string). This silent role="files"
    # companion entry bakes the filename + prompt into a plain-text string so
    # the full indexing pipeline (vanilla, long context, precision) picks it up
    # exactly like an uploaded file.
    #
    # PAIRING NOTE: The companion is stored as role="files" in memory (silent —
    # never sent to the LLM as a duplicate) but indexed into FAISS as role="user".
    # This is critical: the paired-memory backward walk from an assistant turn
    # only stops at role="user" or role="assistant". If the FAISS entry is
    # role="files", the walk skips it and can't find the image user turn.
    # Storing it as role="user" in FAISS makes both forward and backward pairing
    # work correctly without touching any pair-walk logic.
    #
    # _img_filename / _prompt_with_file resolved above (before user payload).
    _img_companion_id   = _new_msg_id()
    _img_companion_text = f"[analyzed image: '{_img_filename}']: {_prompt_with_file}"
    _img_companion = {
        "role":         "files",
        "content":      _img_companion_text,
        "silent":       True,
        "msg_id":       _img_companion_id,
        "timestamp":    datetime.now().isoformat(timespec='seconds'),
        "_img_companion": True,   # guard: skipped by all LLM context builders
    }
    memory.append(_img_companion)
    save_memory(memory)
    print(f"DEBUG: /analyze_image: Saved image companion entry for '{_img_filename}'.")

    # Two-branch indexing — exact mirror of the file upload logic,
    # except role="user" is passed to the indexer (not "files") so that
    # FAISS pair pools see this as the user turn and pairing works in both
    # directions. The in-memory entry stays role="files" (silent).
    _img_session_id = get_current_session_id()
    if (RESONANCE_SETTINGS.get("chunking_enabled", False) and
            RESONANCE_SETTINGS.get("chunk_index_system_immediately", True) and
            RESONANCE_SETTINGS.get("faiss_permanent_indexing", False)):
        try:
            _img_source_label = f"analyzed_image:{hash(_img_filename + prompt[:64]) & 0xFFFFFF}"
            _index_system_content_immediately(
                _img_companion_text,
                "user",            # role="user" in FAISS so pair walks stop here
                _img_source_label,
                _img_session_id,
                pinned=False,
                msg_id=_img_companion_id,
                filename_hint=_img_filename,
            )
        except Exception as _ime:
            print(f"DEBUG (/analyze_image ChunkDLC): Failed to index companion entry: {_ime}")
    elif RESONANCE_SETTINGS.get("always_index_messages", False) and \
         RESONANCE_SETTINGS.get("chunk_index_system_immediately", True):
        # Guard: if chunk_index_system_immediately is OFF, the user explicitly
        # does not want system/injection content indexed — respect that here too.
        try:
            # Pass role="user" + silent=False so _always_index_single_message
            # doesn't skip it (it blocks silent non-files entries).
            _always_index_single_message(
                {**_img_companion, "role": "user", "silent": False},
                _img_session_id,
            )
        except Exception as _ime:
            print(f"DEBUG (/analyze_image AlwaysIndex): Failed to index companion entry: {_ime}")
    # --- END IMAGE COMPANION ENTRY ---
...
# Mirrors /analyze_image but accepts a list of base64 frames extracted
# from a video on the frontend. Each frame becomes an image_url content part
# in OpenAI multipart format — Ollama's /v1/chat/completions compat layer
# maps these to its internal images[] array automatically.
# Other backends (Kobold, LM Studio) will simply ignore the image parts if
# the loaded model doesn't support vision. No backend detection needed.
#
# Frames are stored in memory EXACTLY like analyze_image stores its blob —
# full image_url parts kept intact so KV cache stays warm next turn.
# Costs ~2k tokens to keep vs 4-6k reprocess penalty if stripped.
# Slides out naturally with the sliding window when context fills.
# =============================================================================
...
def analyze_video():
    """
    Accepts:
      - frames:    list of base64 data URLs ["data:image/jpeg;base64,...", ...]
                   Extracted from the video on the frontend (canvas frame grab).
                   Standalone images attached alongside the video are appended
                   to this list by the frontend before sending.
      - prompt:    optional user prompt string.
      - fps_label: optional cosmetic string e.g. "18 frames from clip.mp4"

    Returns a streaming plain-text response identical to /analyze_image.
    """
    data      = request.json or {}
    frames    = data.get("frames", [])
    prompt    = data.get("prompt", "Describe what happens in this video clip in detail.")
    fps_label        = data.get("fps_label", "")
    frame_timestamps = data.get("frame_timestamps", [])

    if not frames:
        return jsonify({"error": "No frames provided. Send at least one base64 frame."}), 400

    # Cap frames to avoid blowing up context on misconfigured clients.
    # VIDEO SETTINGS — server-side hard cap; overridable via /set_video_settings
    MAX_FRAMES = int(VIDEO_SETTINGS.get("frame_cap", 60))
    if len(frames) > MAX_FRAMES:
        print(f"DEBUG: /analyze_video: Capping {len(frames)} frames to {MAX_FRAMES}.")
        step = len(frames) / MAX_FRAMES
        frames = [frames[int(i * step)] for i in range(MAX_FRAMES)]

    # --- Load memory & system prompt (same pattern as /analyze_image) ---
    memory = load_memory()
    system_prompt_message, active_character_name = get_system_prompt()
    global _current_active_character_name
    _current_active_character_name = active_character_name
    ...
    if fps_label:
        display_prompt = f"{prompt}\n[Video frames: {fps_label}]"

    # --- Split into individual messages: 1 user intro + N system frame messages ---
    # Each frame gets its own msg_id so the sliding window can evict them
    # independently instead of nuking the whole video turn at once.
    ...
    for i, frame_data_url in enumerate(frames):
        ...
        frame_payloads.append(frame_msg)
        memory.append(frame_msg)

    save_memory(memory)
    print(f"DEBUG: /analyze_video: Saved 1 user intro + {len(frame_payloads)} frame message(s) to memory.")
    ...
    for msg in active_memory:
        if msg.get("_img_companion"):
            continue  # indexing-only entry — skip in video context too
        msg_for_llm = {k: v for k, v in msg.items() if k not in _INTERNAL_MSG_KEYS}
        if msg_for_llm.get("role") in ("files", "frame", "memory"):
            msg_for_llm["role"] = "system"
        role    = msg_for_llm.get("role")
        ...
        # Name prepending (same logic as /analyze_image)
        if role == "user":
            if name and name.strip():
                if isinstance(content, str):
                    msg_for_llm["content"] = f"{name} {content}"
    ...
    print(f"DEBUG: /analyze_video: Sending {len(frames)} frame(s) to LLM. Prompt: '{prompt[:80]}'")

    # --- Stream response (identical to /analyze_image) ---
    def response_generator():
        full_response_acc = ""
        try:
            # Uses chat_stream temperature from your Temperatures settings — keeps the persona voice consistent
            for chunk in _get_llm_response_stream(messages_to_send_to_llm):
                full_response_acc += chunk
                yield chunk
        except Exception as e:
            print(f"ERROR: /analyze_video streaming failed: {e}")
            yield "Error: Could not analyze the video."
        finally:
            _skip_prefixes = ("Error:", "ERROR:")
            if full_response_acc.strip() and not any(full_response_acc.strip().startswith(p) for p in _skip_prefixes):
                assistant_response_payload = {
                ...
                mem.append(assistant_response_payload)
                save_memory(mem)
                print("DEBUG: /analyze_video: Saved assistant response to memory.")
                if RESONANCE_SETTINGS.get("always_index_messages", False):
                    try:
                        _vid_session_id = get_current_session_id()
                        _always_index_single_message(assistant_response_payload, _vid_session_id)
                    except Exception as _ie:
                        print(f"DEBUG (/analyze_video always-index): Failed to index assistant response: {_ie}")
            else:
                print("DEBUG: /analyze_video: Skipping save — empty or error response.")

    return Response(response_generator(), mimetype='text/plain')

# =============================================================================
# --- END ANALYZE VIDEO ENDPOINT ---
# =============================================================================

# =============================================================================
# --- SERVER-SIDE VIDEO FRAME EXTRACTION ENDPOINT ---
# Accepts a raw video file upload, extracts frames with OpenCV, saves them to
# static/video_frames/<session_id>/ for debugging, then streams the LLM
# response exactly like /analyze_video.
#
# Why: mobile browsers (Chrome/Firefox in mobile mode) have canvas security
# restrictions that produce black frames during client-side extraction.
# Offloading to the server bypasses the browser entirely.
# =============================================================================
import tempfile
...
def upload_video():
    if not _CV2_AVAILABLE:
        return jsonify({"error": "Server-side video extraction unavailable. Install opencv-python-headless."}), 503

    video_file = request.files.get("video")
    if not video_file:
        return jsonify({"error": "No video file provided."}), 400

    prompt       = request.form.get("prompt", "Describe what happens in this video clip in detail.")
    frame_divisor = int(float(request.form.get("frame_divisor", VIDEO_SETTINGS.get("frame_divisor", 2))))
    frame_floor   = int(float(request.form.get("frame_floor",   VIDEO_SETTINGS.get("frame_floor",   8))))
    frame_cap     = int(float(request.form.get("frame_cap",     VIDEO_SETTINGS.get("frame_cap",     200))))
    jpeg_quality  = float(request.form.get("jpeg_quality",      VIDEO_SETTINGS.get("jpeg_quality",  0.82)))
    resolution    = int(float(request.form.get("resolution",    VIDEO_SETTINGS.get("resolution",    720))))

    # How many extra standalone images came along
    extra_image_count = int(request.form.get("extra_image_count", 0))
    extra_images = []
    for i in range(extra_image_count):
        val = request.form.get(f"extra_image_{i}", "")
        if val:
            extra_images.append(val)
    ...
    try:
        ...
        if not cap.isOpened():
            return jsonify({"error": f"Could not open video '{filename}'. Unsupported codec?"}), 400
        ...
        if duration <= 0:
            cap.release()
            return jsonify({"error": f"'{filename}' has no readable duration."}), 400

        # Same frame-count math as the frontend
        frame_divisor = max(1, frame_divisor)
        frame_floor   = max(1, frame_floor)
        raw_count     = max(1, round(duration / frame_divisor))
        num_frames    = min(frame_cap, max(frame_floor, raw_count))

        vid_w  = int(cap.get(cv2.CAP_PROP_FRAME_WIDTH))  or 640
        vid_h  = int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT)) or 360
        scale  = min(1.0, resolution / max(vid_w, vid_h))
        new_w  = max(1, round(vid_w * scale))
        new_h  = max(1, round(vid_h * scale))

        # Spread timestamps across 2%–98% of clip (mirrors frontend logic exactly)
        t_start = min(0.05, duration * 0.02)
        t_end   = duration * 0.98
        if num_frames == 1:
            seek_times = [(t_start + t_end) / 2]
        else:
            seek_times = [t_start + (i / (num_frames - 1)) * (t_end - t_start)
                          for i in range(num_frames)]

        # Create per-session frames dir for debug visibility
        session_id = get_current_session_id()
        ...
        frames           = []
        frame_timestamps = []
        q_int            = max(1, min(100, int(jpeg_quality * 100)))

        for i, ts in enumerate(seek_times):
            frame_pos = int(ts * raw_fps)
            cap.set(cv2.CAP_PROP_POS_FRAMES, frame_pos)
            ret, frame = cap.read()
            if not ret:
                print(f"DEBUG: /upload_video: Frame {i} at {ts:.2f}s unreadable — skipping.")
                continue

            if scale < 1.0:
                frame = cv2.resize(frame, (new_w, new_h), interpolation=cv2.INTER_AREA)

            # Save to disk (handy for debugging on mobile)
            frame_path = os.path.join(frames_dir, f"frame_{i:04d}.jpg")
            cv2.imwrite(frame_path, frame, [cv2.IMWRITE_JPEG_QUALITY, q_int])

            # Encode to base64 for LLM
            _, buf = cv2.imencode('.jpg', frame, [cv2.IMWRITE_JPEG_QUALITY, q_int])
            b64 = base64.b64encode(buf.tobytes()).decode('utf-8')
            frames.append(f"data:image/jpeg;base64,{b64}")
            frame_timestamps.append(ts)

        cap.release()
        print(f"DEBUG: /upload_video: Extracted {len(frames)} frames from '{filename}' → {frames_dir}")
    ...
    if not frames:
        return jsonify({"error": f"No frames could be extracted from '{filename}'."}), 400

    # Append any standalone images that came with the video
    if extra_images:
        frames += extra_images
        print(f"DEBUG: /upload_video: Appended {len(extra_images)} extra image(s).")

    fps_label = f"{len(frames)} frames from '{filename}'" + \
                (f" + {len(extra_images)} image(s)" if extra_images else "")

    # --- From here, identical to /analyze_video ---
    memory = load_memory()
    system_prompt_message, active_character_name = get_system_prompt()
    global _current_active_character_name
    _current_active_character_name = active_character_name
    ...
    # --- Split into individual messages: 1 user intro + N system frame messages ---
    # Mirrors analyze_video: each frame is its own disposable system message
    # so the sliding window can evict frames one by one instead of all-or-nothing.
    _ts_now = datetime.now().isoformat(timespec='seconds')
    user_message_payload = {
        "role":      "user",
        "content":   display_prompt,
    ...
    save_memory(memory)
    print(f"DEBUG: /upload_video: Saved 1 user intro + {len(frame_payloads)} frame message(s) to memory.")
    ...
    messages_to_send_to_llm = [system_prompt_message]
    for msg_entry in active_memory:
        if msg_entry.get("_img_companion"):
            continue  # indexing-only entry
        msg_for_llm = {k: v for k, v in msg_entry.items() if k not in _INTERNAL_MSG_KEYS}
        if msg_for_llm.get("role") in ("files", "frame", "memory"):
            msg_for_llm["role"] = "system"
        role    = msg_for_llm.get("role")
        name    = msg_entry.get("name")
        content = msg_for_llm.get("content")

        if role == "user" and name and name.strip():
            if isinstance(content, str):
                msg_for_llm["content"] = f"{name} {content}"
            elif isinstance(content, list):
                new_parts = []
                for part in content:
                    p = part.copy()
                    if p.get("type") == "text":
                        p["text"] = f"{name} {p.get('text', '')}"
                    new_parts.append(p)
                msg_for_llm["content"] = new_parts
        ...
        if (role == "assistant"
                ...
                and isinstance(msg_for_llm.get("content"), str)
                and "<think>" in msg_for_llm["content"]):
            _m = re.search(r'<think>(.*?)</think>', msg_for_llm["content"], re.DOTALL)
            if _m:
                msg_for_llm["content"]           = re.sub(r'<think>.*?</think>', '', msg_for_llm["content"], flags=re.DOTALL).strip()
                msg_for_llm["reasoning_details"] = [{"type": "thinking", "thinking": _m.group(1).strip()}]

        messages_to_send_to_llm.append(msg_for_llm)

    print(f"DEBUG: /upload_video: Sending {len(frames)} frame(s) to LLM.")
    ...
    def response_generator():
        ...
        try:
            ...
            print(f"ERROR: /upload_video streaming failed: {e}")
            yield "Error: Could not analyze the video."
        finally:
            _skip = ("Error:", "ERROR:")
            if full_response_acc.strip() and not any(full_response_acc.strip().startswith(p) for p in _skip):
                assistant_payload = {
                    "role":      "assistant",
                    "name":      active_character_name,
                    "content":   full_response_acc,
                ...
                mem = load_memory()
                mem.append(assistant_payload)
                save_memory(mem)
                print("DEBUG: /upload_video: Saved assistant response to memory.")
                if RESONANCE_SETTINGS.get("always_index_messages", False):
                    try:
                        _always_index_single_message(assistant_payload, get_current_session_id())
                    except Exception as _ie:
                        print(f"DEBUG (/upload_video always-index): {_ie}")
            else:
                print("DEBUG: /upload_video: Skipping save — empty or error response.")

    return Response(response_generator(), mimetype='text/plain')

# =============================================================================
# --- END SERVER-SIDE VIDEO FRAME EXTRACTION ENDPOINT ---
# =============================================================================
...
def new_session():
    ...
    # --- Auto-inject first_mes / scenario for the active character card ---
    # If the currently active persona is a PNG character card with a cached first_mes,
    # seed the fresh session with it so the card always opens with its greeting.
    has_first_mes = False
    try:
        _, active_persona_name = get_system_prompt()
        if active_persona_name:
            persona_entry = PERSONAS.get(active_persona_name, {})
            if isinstance(persona_entry, dict) and persona_entry.get("is_character_card"):
                first_mes = persona_entry.get("first_mes", "").strip()
                if first_mes:
                    seed_memory = [{
                        "role": "assistant",
                        "name": active_persona_name,
                        "content": first_mes,
                        "timestamp": datetime.now().isoformat(timespec='seconds'),
                        "msg_id": _new_msg_id()
                    }]
                    save_memory(seed_memory)
                    has_first_mes = True
                    print(f"DEBUG (CharCard): Seeded new session '{new_id}' with first_mes for '{active_persona_name}'.")
                else:
                    save_memory([])
            else:
                save_memory([])
        ...
        print(f"WARNING (new_session): Could not inject first_mes: {e}")
        save_memory([])

    # Kick off background index work so global index is warm before the first recall.
    def _bg_index_new():
        _set_rebuild_running(True)
        try:
            print(f"DEBUG (Faiss): Background index check for new session '{new_id}'.")
            mem = load_memory()
            active = ghost_memory_if_needed(mem)
            check_and_update_faiss_index(mem, active, new_id)
            if RESONANCE_SETTINGS.get("global_memory_enabled", False):
                print(f"DEBUG (Faiss): Global memory ON — syncing global index after new session '{new_id}'.")
                rebuild_global_index()
        except Exception as _e:
            print(f"DEBUG (Faiss): Background index check error for new session '{new_id}': {_e}")
        finally:
            _set_rebuild_running(False)

    threading.Thread(target=_bg_index_new, daemon=True).start()
    ...
    return jsonify({"message": f"Switched to new session: {new_id}", "session_id": new_id, "has_first_mes": has_first_mes}), 200
...
def switch_session():
    ...
    def _bg_index():
        ...
        try:
            ...
            # If global memory is ON, sync the global index every session switch
            # so the first recall in the new session always hits a warm index.
            if RESONANCE_SETTINGS.get("global_memory_enabled", False):
                print(f"DEBUG (Faiss): Global memory ON — syncing global index after switch to '{session_id}'.")
                rebuild_global_index()
        except Exception as _e:
...
def api_set_system_prompt():
    ...
    # --- Character card: greeting injection / picker ---
    has_first_mes       = False
    needs_greeting_pick = False
    greetings_list      = []

    persona_entry = PERSONAS.get(persona_name, {})
    if isinstance(persona_entry, dict) and persona_entry.get("is_character_card") and make_active:
        live_user  = get_current_user_loadout_name() or "User"
        char_nick  = persona_entry.get("char_nickname", persona_name)
        first_mes  = _resolve_card_templates(persona_entry.get("first_mes", ""), char_nick, live_user)
        alt_greetings = [_resolve_card_templates(g, char_nick, live_user)
                         for g in persona_entry.get("alt_greetings", [])]

        all_greetings = ([first_mes] if first_mes else []) + alt_greetings

        if len(all_greetings) > 1:
            # Multiple greetings — let frontend show the picker
            needs_greeting_pick = True
            greetings_list = [g[:120] + ("..." if len(g) > 120 else "") for g in all_greetings]
        elif len(all_greetings) == 1:
            # Only one greeting — auto inject, no picker needed
            try:
                memory = load_memory()
                already_there = (
                    memory
                    and memory[0].get("role") == "assistant"
                    and memory[0].get("content", "").strip() == all_greetings[0]
                )
                if not already_there:
                    if memory and memory[0].get("role") == "assistant" and isinstance(PERSONAS.get(memory[0].get("name")), dict) and PERSONAS[memory[0].get("name")].get("is_character_card"):
                        memory.pop(0)
                    memory.insert(0, {
                        "role":      "assistant",
                        "name":      persona_name,
                        "content":   all_greetings[0],
                        "timestamp": datetime.now().isoformat(timespec='seconds'),
                        "msg_id":    _new_msg_id()
                    })
                    save_memory(memory)
                    has_first_mes = True
                    print(f"DEBUG (CharCard): Injected greeting for '{persona_name}' on persona switch.")
            except Exception as e:
                print(f"WARNING (CharCard): Could not inject greeting on switch: {e}")
    ...
    return jsonify({
        ...
        "has_first_mes":         has_first_mes,
        "needs_greeting_pick":   needs_greeting_pick,
        "greetings":             greetings_list,
        "persona_name":          persona_name
    })


@app.route("/get_character_greetings", methods=["POST"])
def get_character_greetings():
    data = request.json or {}
    persona_name = data.get("persona_name", "").strip()
    entry = PERSONAS.get(persona_name, {})
    if not isinstance(entry, dict) or not entry.get("is_character_card"):
        return jsonify({"error": "Not a character card."}), 400
    live_user = get_current_user_loadout_name() or "User"
    char_nick  = entry.get("char_nickname", persona_name)
    first_mes     = _resolve_card_templates(entry.get("first_mes", ""), char_nick, live_user)
    alt_greetings = [_resolve_card_templates(g, char_nick, live_user) for g in entry.get("alt_greetings", [])]
    greetings = ([first_mes] if first_mes else []) + alt_greetings
    return jsonify({"persona_name": persona_name, "greetings": greetings, "count": len(greetings)})


@app.route("/inject_greeting", methods=["POST"])
def inject_greeting():
    data = request.json or {}
    persona_name   = data.get("persona_name", "").strip()
    greeting_index = int(data.get("greeting_index", 0))
    entry = PERSONAS.get(persona_name, {})
    if not isinstance(entry, dict) or not entry.get("is_character_card"):
        return jsonify({"error": "Not a character card."}), 400
    live_user  = get_current_user_loadout_name() or "User"
    ...
    greetings = ([first_mes] if first_mes else []) + alt_greetings
    if not greetings:
        return jsonify({"error": "This card has no greetings."}), 400
    idx    = max(0, min(greeting_index, len(greetings) - 1))
    chosen = greetings[idx]
    try:
        memory = load_memory()
        if memory and memory[0].get("role") == "assistant":
            top_name = memory[0].get("name", "")
            if isinstance(PERSONAS.get(top_name), dict) and PERSONAS[top_name].get("is_character_card"):
                memory.pop(0)
        memory.insert(0, {
            "role":      "assistant",
            "name":      persona_name,
        ...
        return jsonify({"error": f"Could not inject greeting: {e}"}), 500
    print(f"DEBUG (CharCard): Injected greeting [{idx}] for '{persona_name}'.")
    return jsonify({"message": f"Greeting {idx} injected.", "greeting_index": idx, "preview": chosen[:80] + ("..." if len(chosen) > 80 else "")})
...
def set_resonance_settings():
    ...
    if _old_global_enabled and not _new_global_enabled:
        ...
        if _sid:
            # FIX (ghost stamp): same as set_faiss_settings — clear local flags so
            # the bg rebuild actually writes messages into local instead of printing
            # CLEAN on an empty index. See set_faiss_settings comment for full detail.
            _local_flag = _get_local_flag_key()
            _clear_msg_flags(_sid, _local_flag)
            print(f"DEBUG (set_resonance_settings): Cleared '{_local_flag}' ghost stamps "
                  f"from '{_sid}' so bg rebuild can re-index into local.")
            def _bg_index_rebuild(_sid=_sid):
                ...
                try:
                    ...
                    print(f"DEBUG (set_resonance_settings): bg index rebuild error: {_e}")
                finally:
                    _set_rebuild_running(False)
    ...
    for key, cast in type_map.items():
        if key in data:
            try:
                raw = data[key]
                if cast is bool:
                    # Safely coerce both JSON booleans and string representations.
                    # Python's built-in bool("false") returns True (truthy string),
                    # which would silently re-enable the setting every time it's
                    # saved as OFF.  Normalise to a real Python bool first.
                    if isinstance(raw, bool):
                        RESONANCE_SETTINGS[key] = raw
                    elif isinstance(raw, str):
                        RESONANCE_SETTINGS[key] = raw.strip().lower() not in ("false", "0", "no", "off", "")
                    else:
                        RESONANCE_SETTINGS[key] = bool(raw)
                else:
                    RESONANCE_SETTINGS[key] = cast(raw)
            except (ValueError, TypeError):
                return jsonify({"error": f"Invalid value for {key}"}), 400
    save_resonance_settings()

    # DEBUG: log the final bool values so toggling issues are immediately visible
    _bool_keys = [k for k, v in type_map.items() if v is bool]
    for _bk in _bool_keys:
        print(f"DEBUG (set_chunking_settings): {_bk} = {RESONANCE_SETTINGS.get(_bk)}")
...
@app.route("/get_video_settings", methods=["GET"])
def get_video_settings():
    """Returns the persisted video analysis settings."""
    return jsonify(VIDEO_SETTINGS)


@app.route("/set_video_settings", methods=["POST"])
def set_video_settings():
    """
    Saves video analysis settings to video_config.json.

    Accepted keys (all optional — send only what changed):
      frame_divisor  int    — 1 frame sampled per N seconds (1–10)
      frame_floor    int    — minimum frames for short clips (1–20)
      frame_cap      int    — server-side hard cap on total frames (4–120)
      jpeg_quality   float  — JPEG compression quality (0.1–1.0)
      resolution     int    — MAX_DIM longest-edge cap in px (256/512/720/1080)
      temperature    float  — LLM temperature for analysis (0.0–2.0)
    ...
    ALLOWED_KEYS = {"frame_divisor", "frame_floor", "frame_cap",
                    "jpeg_quality", "resolution"}
    unknown = set(data.keys()) - ALLOWED_KEYS
    if unknown:
        return jsonify({
            "error": f"Unknown video setting key(s): {sorted(unknown)}. "
                     f"Allowed: {sorted(ALLOWED_KEYS)}"
        }), 400

    # Type coercions + range clamps
    try:
        if "frame_divisor" in data:
            data["frame_divisor"] = max(1, min(10, int(data["frame_divisor"])))
        if "frame_floor" in data:
            data["frame_floor"] = max(1, min(20, int(data["frame_floor"])))
        if "frame_cap" in data:
            data["frame_cap"] = max(4, min(300, int(data["frame_cap"])))  # hard ceiling raised from 120 → 300
        if "jpeg_quality" in data:
            data["jpeg_quality"] = max(0.1, min(1.0, float(data["jpeg_quality"])))
        if "resolution" in data:
            allowed_res = {256, 512, 720, 1080}
            val = int(data["resolution"])
            if val not in allowed_res:
                return jsonify({"error": f"resolution must be one of {sorted(allowed_res)}, got {val}"}), 400
            data["resolution"] = val
    except (TypeError, ValueError) as e:
        return jsonify({"error": f"Type error in video settings: {e}"}), 400

    VIDEO_SETTINGS.update(data)
    save_video_settings()
    return jsonify({"status": "ok", **VIDEO_SETTINGS})

# =============================================================================
# --- END VIDEO SETTINGS ENDPOINTS ---
# =============================================================================
...
def set_faiss_settings():
    ...
    if save_needed:
        ...
        # FIX BUG (set_faiss_settings): when global_memory_enabled is flipped OFF,
        # warm the local index so recall doesn't land on an empty/stale index.
        # Mirrors the identical guard already in set_resonance_settings (FIX BUG 3).
        _new_global_enabled = RESONANCE_SETTINGS.get("global_memory_enabled", False)
        if not _new_global_enabled and not global_was_off:  # just turned OFF
            _sid = get_current_session_id()
            if _sid:
                # FIX (ghost stamp): While global was ON, _always_index_single_message
                # stamped indexed_local_longctx=True on every message as a side effect of
                # the dual-stamp BACKLOG fix — but it never actually wrote those messages
                # into the local index file (global path returns early after writing global).
                # Without this pre-clear, check_and_update_faiss_index sees all local flags
                # already set, treats everything as "done", prints CLEAN, and leaves the
                # local index cold. The local RAG lane then finds nothing after the switch.
                # Solution: wipe the local flags for this session before the rebuild fires
                # so the flush-check rediscovers every message and writes them into local.
                _local_flag = _get_local_flag_key()
                _clear_msg_flags(_sid, _local_flag)
                print(f"DEBUG (set_faiss_settings): Cleared '{_local_flag}' ghost stamps "
                      f"from '{_sid}' so bg rebuild can re-index into local.")
                def _bg_local_rebuild(_sid=_sid):
                    _set_rebuild_running(True)
                    try:
                        _mem = load_memory()
                        _active = ghost_memory_if_needed(_mem)
                        ...
                        print(f"DEBUG (set_faiss_settings): bg local rebuild error: {_e}")
                    finally:
                        _set_rebuild_running(False)
                threading.Thread(target=_bg_local_rebuild, daemon=True).start()
                print("DEBUG (set_faiss_settings): global_memory_enabled flipped OFF → "
                      "triggered background local index rebuild.")
        ...
        if should_rebuild:
            ...
            def _bg_global_rebuild():
                _set_rebuild_running(True)
                try:
                    rebuild_global_index(force=True)
                except Exception as _e:
                    print(f"DEBUG (Global Index): bg global rebuild error: {_e}")
                finally:
                    _set_rebuild_running(False)
            threading.Thread(target=_bg_global_rebuild, daemon=True).start()
            return jsonify({
    ...
    def _do_rebuild():
        ...
        try:
            if scope in ("local", "all"):
                ...
                # skip_global=True because scope="all" will rebuild global explicitly below,
                # preventing rebuild_global_index() from running twice.
                check_and_update_faiss_index(memory, active_memory, session_id,
                                             skip_global=(scope == "all"))
...
def _resolve_card_templates(text: str, char_name: str, user_name: str) -> str:
    """
    Resolves all SillyTavern / CCv3 Curly Brace Syntaxes (CBS) in card text.
    Handled tokens (all case-insensitive):
      {{char}} / {{character}}   → char_name (nickname or name)
      {{user}}                   → user_name
      {{chat}}                   → '' (no chat-name concept in this app)
      {{original}}               → '' (no global jailbreak concept)
      {{// ...}}                 → '' (invisible comment)
      {{comment: ...}}           → '' (inline comment)
      {{random:A,B,C}}           → random pick from options
      {{pick:A,B,C}}             → random pick (alias)
      {{roll:N}} / {{roll:dN}}   → random int between 1 and N
      {{reverse:A}}              → reversed string of A
    """
    if not text:
        return text
    import re as _re
    ...
    text = _re.sub(r'\{\{comment:[^}]*\}\}', '', text, flags=_re.IGNORECASE)

    def _replace_random(m):
        raw = m.group(1)
        parts = _re.split(r'(?<!\\),', raw)
        parts = [p.replace(r'\,', ',').strip() for p in parts if p.strip()]
        return _random.choice(parts) if parts else ''
    text = _re.sub(r'\{\{(?:random|pick):([^}]+)\}\}', _replace_random, text, flags=_re.IGNORECASE)

    def _replace_roll(m):
        raw = m.group(1).lstrip('dD')
        try:
            n = int(raw)
            return str(_random.randint(1, max(1, n)))
        except ValueError:
            return '1'
    text = _re.sub(r'\{\{roll:([^}]+)\}\}', _replace_roll, text, flags=_re.IGNORECASE)

    def _replace_reverse(m):
        return m.group(1)[::-1]
    text = _re.sub(r'\{\{reverse:([^}]*)\}\}', _replace_reverse, text, flags=_re.IGNORECASE)

    text = _re.sub(r'\{\{char(?:acter)?\}\}', char_name, text, flags=_re.IGNORECASE)
    text = _re.sub(r'\{\{user\}\}',            user_name,  text, flags=_re.IGNORECASE)
    text = _re.sub(r'\{\{chat\}\}',            '',         text, flags=_re.IGNORECASE)
    text = _re.sub(r'\{\{original\}\}',        '',         text, flags=_re.IGNORECASE)
...
def import_character_card():
    """
    Imports a SillyTavern-format character card PNG.
    Extracts the embedded 'chara' tEXt chunk (base64 JSON),
    creates a persona from the character data, and saves the PNG as avatar.
    Supports both V1 ({name, description, ...}) and V2 ({data: {name, ...}}) card formats.
    Also handles zTXt compressed chunks in addition to plain tEXt.
    """
    import struct
    import zlib
    ...
    # --- Parse PNG chunks — prefer 'ccv3' (V3 spec), fall back to 'chara' (V1/V2) ---
    char_json = None
    ccv3_json = None
    chara_json = None
    try:
        ...
        while pos < len(png_bytes):
            ...
            if chunk_type == "tEXt":
                ...
                if null_idx != -1:
                    ...
                    text_value = chunk_data[null_idx+1:].decode("latin-1", errors="ignore")
                    if keyword == "ccv3" and ccv3_json is None:
                        try:
                            ccv3_json = json.loads(base64.b64decode(text_value).decode("utf-8"))
                        except Exception:
                            pass
                    elif keyword == "chara" and chara_json is None:
                        try:
                            chara_json = json.loads(base64.b64decode(text_value).decode("utf-8"))
                        except Exception:
                            pass
                ...
                if null_idx != -1:
                    keyword = chunk_data[:null_idx].decode("ascii", errors="ignore")
                    if keyword in ("ccv3", "chara") and null_idx + 2 <= len(chunk_data):
                        compressed = chunk_data[null_idx+2:]
                        try:
                            text_value = zlib.decompress(compressed).decode("utf-8")
                            parsed = json.loads(base64.b64decode(text_value).decode("utf-8"))
                            if keyword == "ccv3" and ccv3_json is None:
                                ccv3_json = parsed
                            elif keyword == "chara" and chara_json is None:
                                chara_json = parsed
        ...
        # V3 wins if both present
        char_json = ccv3_json if ccv3_json is not None else chara_json

    except Exception as e:
        return jsonify({"error": f"Failed to parse PNG chunks: {e}"}), 400

    if not char_json:
        return jsonify({"error": "No character data found. Make sure it's a valid character card PNG."}), 400

    # --- Extract fields (V1 flat, V2/V3 nested under 'data') ---
    card_data = char_json.get("data", char_json)

    name        = (card_data.get("name") or char_json.get("name") or "Imported Character").strip()
    # V3: nickname overrides {{char}}, falls back to name if absent/empty
    nickname    = (card_data.get("nickname") or "").strip() or name

    description           = (card_data.get("description") or "").strip()
    personality           = (card_data.get("personality") or "").strip()
    scenario              = (card_data.get("scenario") or "").strip()
    first_mes             = (card_data.get("first_mes") or "").strip()
    system_prompt_field   = (card_data.get("system_prompt") or "").strip()
    post_history_instruct = (card_data.get("post_history_instructions") or "").strip()
    mes_example           = (card_data.get("mes_example") or "").strip()

    raw_alts      = card_data.get("alternate_greetings") or []
    alt_greetings = [g.strip() for g in raw_alts if isinstance(g, str) and g.strip()]

    # --- Resolve CBS tokens (use nickname as {{char}} per spec) ---
    # IMPORTANT: Pass "{{user}}" as the user_name sentinel so the token is preserved
    # literally in the stored prompt rather than baked in at import time.
    # {{user}} is resolved live in get_system_prompt() from the active loadout,
    # so it always reflects whoever is currently active — no stale names.
    _sentinel = "{{user}}"
    description           = _resolve_card_templates(description,           nickname, _sentinel)
    personality           = _resolve_card_templates(personality,           nickname, _sentinel)
    scenario              = _resolve_card_templates(scenario,              nickname, _sentinel)
    first_mes             = _resolve_card_templates(first_mes,             nickname, _sentinel)
    system_prompt_field   = _resolve_card_templates(system_prompt_field,   nickname, _sentinel)
    post_history_instruct = _resolve_card_templates(post_history_instruct, nickname, _sentinel)
    alt_greetings         = [_resolve_card_templates(g, nickname, _sentinel) for g in alt_greetings]
    ...
    if system_prompt_field:
        prompt_parts.append(f"[System]: {system_prompt_field}")
    if post_history_instruct:
        prompt_parts.append(f"[Post Instructions]: {post_history_instruct}")
    if mes_example:
        prompt_parts.append(f"[Example Dialogue]:\n{mes_example}")

    system_prompt = "\n\n".join(prompt_parts) if prompt_parts else f"You are {nickname}."

    # --- Deduplicate name if one already exists ---
    final_name = name
    if final_name in PERSONAS:
        suffix = 1
        while f"{name}_{suffix}" in PERSONAS:
            suffix += 1
        final_name = f"{name}_{suffix}"

    # --- Save persona ---
    personas_copy = PERSONAS.copy()
    personas_copy[final_name] = {
        "prompt":            system_prompt,
        "avatar_image":      None,
        "is_character_card": True,
        "char_nickname":     nickname,       # the {{char}} value per spec
        "first_mes":         first_mes,      # already resolved at import
        "alt_greetings":     alt_greetings,  # stored for picker
    }

    # --- Save avatar image (reuse same naming convention as upload_persona_avatar) ---
    try:
        safe_name = re.sub(r'[^\w]', '_', final_name)
        _name_hash = format(hash(final_name) & 0xFFFFFF, '06x')
        avatar_filename = f"persona_{safe_name}_{_name_hash}.png"
        avatar_path = os.path.join(PERSONA_IMAGES_DIR, avatar_filename)
        with open(avatar_path, "wb") as f:
            f.write(png_bytes)
        personas_copy[final_name]["avatar_image"] = avatar_filename
        print(f"DEBUG (CharCard): Avatar saved → {avatar_filename}")
    except Exception as e:
        print(f"WARNING (CharCard): Could not save avatar: {e}")

    save_all_personas(personas_copy)
    set_current_persona_name(final_name)  # activate it immediately on import

    # --- Inject first_mes as visible intro message if present (single greeting only) ---
    # If card has alt_greetings, the frontend will show the picker instead.
    all_greetings = ([first_mes] if first_mes else []) + alt_greetings
    if len(all_greetings) == 1 and first_mes:
        try:
            memory = load_memory()
            memory.insert(0, {
                "role": "assistant",
            ...
            print(f"WARNING (CharCard): Could not inject first_mes: {e}")

    print(f"DEBUG (CharCard): Imported '{final_name}' from character card ({len(all_greetings)} greeting(s)).")
    return jsonify({
        "message":             f"Character '{final_name}' imported successfully!",
        "persona_name":        final_name,
        "has_first_mes":       bool(first_mes),
        "needs_greeting_pick": len(all_greetings) > 1,
        "greetings":           [g[:120] + ("..." if len(g) > 120 else "") for g in all_greetings],
    })
...
# =============================================================================
# --- MEMORY PRESETS (World Info / Prebaked Memories) ---
# Server-side persisted JSON. Each preset: {id, name, content}
# =============================================================================

MEMORY_PRESETS_FILE = 'memory_presets.json'
_memory_presets_lock = threading.Lock()


def _load_memory_presets() -> list:
    """Load presets from disk. Returns [] if file missing or corrupt."""
    if not os.path.exists(MEMORY_PRESETS_FILE):
        return []
    try:
        with open(MEMORY_PRESETS_FILE, 'r', encoding='utf-8') as f:
            data = json.load(f)
            return data if isinstance(data, list) else []
    except Exception as e:
        print(f"WARNING (memory_presets): Could not read presets file: {e}")
        return []


def _save_memory_presets(presets: list):
    """Atomically write presets to disk."""
    tmp = MEMORY_PRESETS_FILE + '.tmp'
    with open(tmp, 'w', encoding='utf-8') as f:
        json.dump(presets, f, indent=2, ensure_ascii=False)
    _safe_replace(tmp, MEMORY_PRESETS_FILE)


@app.route('/get_memory_presets', methods=['GET'])
def get_memory_presets():
    """Returns all saved memory presets."""
    with _memory_presets_lock:
        return jsonify({'presets': _load_memory_presets()})


@app.route('/save_memory_preset', methods=['POST'])
def save_memory_preset_route():
    """Creates a new memory preset. Body: {name, content}"""
    data = request.json or {}
    name = data.get('name', '').strip()
    ...
    with _memory_presets_lock:
        presets = _load_memory_presets()
        preset = {'id': uuid.uuid4().hex[:12], 'name': name, 'content': content}
        presets.append(preset)
        _save_memory_presets(presets)
    print(f"DEBUG (memory_presets): Saved preset '{name}'.")
    return jsonify({'message': f"Preset '{name}' saved.", 'preset': preset})


@app.route('/update_memory_preset', methods=['POST'])
def update_memory_preset_route():
    """Updates an existing memory preset. Body: {id, name?, content?}"""
    data = request.json or {}
    ...
    with _memory_presets_lock:
        ...
        for p in presets:
            if p['id'] == pid:
                ...
                _save_memory_presets(presets)
                print(f"DEBUG (memory_presets): Updated preset '{p['name']}'.")
                return jsonify({'message': f"Preset updated.", 'preset': p})
    return jsonify({'error': 'Preset not found'}), 404


@app.route('/delete_memory_preset', methods=['POST'])
def delete_memory_preset_route():
    """Deletes a memory preset. Body: {id}"""
    data = request.json or {}
    pid = data.get('id', '').strip()
    if not pid:
        return jsonify({'error': 'id is required'}), 400
    with _memory_presets_lock:
        presets = _load_memory_presets()
        new_presets = [p for p in presets if p['id'] != pid]
        if len(new_presets) == len(presets):
            return jsonify({'error': 'Preset not found'}), 404
        _save_memory_presets(new_presets)
    print(f"DEBUG (memory_presets): Deleted preset id='{pid}'.")
