...
def api_set_system_prompt():
    ...
    # --- Character card: re-inject first_mes when switching to this persona ---
    # If the persona being activated is a PNG character card that has a cached
    # first_mes, inject it as the opening assistant message in the current session
    # (unless it's already there from a previous activation of the same card).
    has_first_mes = False
    ...
    if isinstance(persona_entry, dict) and persona_entry.get("is_character_card") and make_active:
        ...
        if first_mes:
            try:
                memory = load_memory()
                # Strip any existing card first_mes at position 0 (from this card OR a
                # previously active card) so switching A->B never stacks messages.
                if memory:
                    top = memory[0]
                    top_name = top.get("name", "")
                    top_is_card = (
                        top.get("role") == "assistant"
                        and isinstance(PERSONAS.get(top_name), dict)
                        and PERSONAS[top_name].get("is_character_card")
                    )
                    if top_is_card:
                        memory.pop(0)

                # Now inject this card's first_mes at the top (always fresh after cleanup)
                memory.insert(0, {
                    "role": "assistant",
                    "name": persona_name,
                    "content": first_mes,
                ...
                save_memory(memory)
                has_first_mes = True
                print(f"DEBUG (CharCard): Injected first_mes for '{persona_name}' on persona switch.")
            except Exception as e:
                print(f"WARNING (CharCard): Could not inject first_mes on switch: {e}")
...
def import_character_card():
    ...
    # --- Parse PNG tEXt chunks to find 'chara' ---
    char_json = None
    try:
        ...
        while pos < len(png_bytes):
            ...
            if chunk_type == "tEXt":
                ...
                if null_idx != -1:
                    ...
                    if keyword == "chara":
                        char_json = json.loads(base64.b64decode(text_value).decode("utf-8"))
                        break
                ...
                if null_idx != -1:
                    keyword = chunk_data[:null_idx].decode("ascii", errors="ignore")
                    if keyword == "chara" and null_idx + 2 <= len(chunk_data):
    ...
    if not char_json:
        return jsonify({"error": "No character data found in this PNG. Make sure it's a valid SillyTavern character card."}), 400

    # --- Extract character fields (supports both V1 and V2 spec) ---
    ...
    # Build a coherent system prompt from all available fields
    prompt_parts = []
    if description:
    ...
    system_prompt = "\n\n".join(prompt_parts) if prompt_parts else f"You are {name}."
    ...
    personas_copy[final_name] = {
        ...
        "first_mes": first_mes  # cached so switching to this persona re-injects it
    }
    ...
    # --- Inject first_mes as visible intro message if present ---
    if first_mes:
        try:
            memory = load_memory()
            memory.insert(0, {
    ...
    print(f"DEBUG (CharCard): Imported '{final_name}' from character card.")
    return jsonify({
