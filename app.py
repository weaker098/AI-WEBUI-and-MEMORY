...
def _backfill_msg_ids(memory: list) -> bool:
    ...
    dirty = False
    for msg in memory:
        if msg.get("role") in ("user", "assistant") and "msg_id" not in msg:
...
def add_message(session_id, role, content, silent=False):
    ...
    if silent:
        ...
        # Stamp a unique ID on every real user/assistant message at birth
...
def get_search_results():
    ...
    if backfill_dirty:
        save_memory(memory)
        print(f"DEBUG (get_search_results): Backfilled msg_id on {sum(1 for r in results if True)} search result(s).")
