...
def _backfill_msg_ids(memory: list) -> bool:
    ...
    dirty = False
    for msg in memory:
        if msg.get("role") in ("user", "assistant", "files") and "msg_id" not in msg:
...
def add_message(session_id, role, content, silent=False):
    ...
    # flag is never set, causing the quota system to re-process them every turn.
...
def check_and_update_faiss_index(memory, active_memory, session_id):
    ...
    try:
        ...
        # 4. Find *new* ghosted messages not yet flagged as indexed_local
        # Primary check: indexed_local flag on the source message (fast, reliable).
        # Fallback 1: content-set for any pre-ID messages that slipped through backfill.
        # Fallback 2: msg_id-set — chunks carry parent msg_id so bouncer can recognise
        #             a sliced message even when content strings never match the full msg.
        indexed_message_contents = {msg.get("content") for msg in indexed_messages}
        indexed_message_ids      = {msg.get("msg_id") for msg in indexed_messages if msg.get("msg_id")}  # FIX: chunk-aware ID set
        ...
        if len(new_ghosted_messages) >= quota or (permanent_index is None and len(new_ghosted_messages) > 0):
            ...
            if RESONANCE_SETTINGS.get("chunking_enabled", False):
                ...
                if all_chunk_entries:
                    ...
                    if permanent_index is None:
                        permanent_index = create_faiss_index(EMBEDDING_DIM)
                    permanent_index, _ivf_fell_back = _maybe_train_ivf(permanent_index, new_embeddings_np)
                    if _ivf_fell_back:
                        print("DEBUG (Faiss + ChunkDLC): IVF fallback to Flat on quota path (not enough vectors).")
                    permanent_index.add(new_embeddings_np)
                    indexed_messages.extend(all_chunk_entries)
                    _atomic_index_write(permanent_index, index_filepath, metadata_filepath, indexed_messages)
                    # FIX BUG 6: Stamp indexed_local on source messages (by parent msg_id)
                    # AND on precision-mode chunk entries (which have their own fresh msg_id).
                    # Previously only source IDs were stamped — precision chunk entries'
                    # new msg_ids were never flagged, so the indexer re-processed them on
                    # every subsequent call instead of treating them as already indexed.
                    _quota_ids = {m.get("msg_id") for m in new_ghosted_messages if m.get("msg_id")}
                    if _precision_mode:
                        # Add chunk entry IDs too so they are deduped correctly
                        _quota_ids.update(e.get("msg_id") for e in all_chunk_entries if e.get("msg_id"))
                    if _quota_ids:
                        _stamp_msg_flags(session_id, _quota_ids, _get_local_flag_key())
            ...
            permanent_index, _ivf_fell_back = _maybe_train_ivf(permanent_index, new_embeddings_np)
            if _ivf_fell_back:
                print("DEBUG (Faiss): IVF fallback to Flat on quota path (not enough vectors).")
            
            permanent_index.add(new_embeddings_np)
            indexed_messages.extend(new_ghosted_messages)
            
            _atomic_index_write(permanent_index, index_filepath, metadata_filepath, indexed_messages)
            # Stamp indexed_local on all newly indexed messages
            _quota_ids = {m.get("msg_id") for m in new_ghosted_messages if m.get("msg_id")}
            if _quota_ids:
                _stamp_msg_flags(session_id, _quota_ids, _get_local_flag_key())
           ...
           RESONANCE_SETTINGS.get("chunk_retrieval_mode", "precision") == "long_context":
            ...
            if RESONANCE_SETTINGS.get("paired_memory_enabled", False) and reformed_messages:
                ...
                for r_msg in reformed_messages:
                    ...
                    if mem_idx is None:
                        # Can't locate in pool — inject the reformed message alone
                        _add_pair(r_msg)
                        continue
...
def _dedup_recall_block(new_reflection, memory, extra_active_contents=None, session_id=None):
    ...
    Chunks inherit their parent's msg_id at index time (see always_index / quota paths),
...
def chat_stream():
    ...
    if file_data and 'image' not in file_data.get('type', ''):
        ...
        if filename and content:
            ...
            # entry hasn't been saved yet, the flag is silently skipped and the quota
            # system will treat the file as unindexed on the next turn, leading to a
...
def delete_message():
    ...
    try:
        ...
        with _memory_lock:
            ...
            # Snapshot msg_ids BEFORE the deletion loop — indices are still valid here.
            # After del memory[i] runs the positions shift, so reading memory[i] afterward
            # would silently return the wrong entry (or raise IndexError at the tail).
            deleted_ids = {
                memory[i].get("msg_id")
                for i in indices_to_delete
                if memory[i].get("msg_id")
            }
        ...
        # deleted_ids was captured before the loop — safe to use here for FAISS flag cleanup
...
def set_resonance_settings():
    ...
    _resonance_defaults_guard = {
        "rag_ghost_preservation": True,
        "persistent_memory_injection": True,
        "recall_injection_position": "after",
    }
    for _k, _v in _resonance_defaults_guard.items():
