...
def initialize_personas():
    ...
    # Ensure default "Rivet" persona exists
    if "Rivet" not in PERSONAS:
        default_prompt_content = (
            "You are Rivet A sassy Feline like Vixen species called Lombax"
        )
        PERSONAS["Rivet"] = {'prompt': default_prompt_content, 'avatar_image': None}
        if not os.path.exists(CURRENT_SYSTEM_PROMPT_NAME_FILE):
            set_current_persona_name("Rivet")
        save_all_personas(PERSONAS)
        print("DEBUG: Default 'Rivet' persona created and saved.")
...
def _migrate_personas_to_dict():
    ...
    Old: { "Rivet": "You are Rivet..." }
    New: { "Rivet": { "prompt": "You are Rivet...", "avatar_image": null } }
    """
    global PERSONAS
    changed = False
...
def call_Rivet_stream(messages, temperature=0.4):
...
def _get_llm_response_stream(messages_to_send_to_llm, temperature=None):
    ...
    try:
        ...
        for chunk in call_Rivet_stream(messages_to_send_to_llm, temp):
            yield chunk
    except Exception as e:
...
def _always_index_single_message(msg, session_id):
    ...
    # Skip silent, system, or empty messages
    ...
    # NOTE: Global index sync is intentionally NOT triggered here.
    # check_and_update_faiss_index() runs on the same turn and will handle
    # global sync if needed — exactly like the quota path does. This prevents
    # a full global rebuild on every single message. 🥔
...
def check_and_update_faiss_index(memory, active_memory, session_id):
    ...
    try:
        ...
        if len(new_ghosted_messages) >= quota or (permanent_index is None and len(new_ghosted_messages) > 0):
            # --- QUOTA MET OR FRESH START: Update permanent index ---
